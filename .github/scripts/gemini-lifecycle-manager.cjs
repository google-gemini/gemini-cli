...
module.exports = async ({ github, context, core }) => {
  ...
  const EXEMPT_LABELS = ['pinned', 'security', '🔒 maintainer only', 'help wanted', '🗓️ Public Roadmap'];
  ...
  await processItems(`repo:${owner}/${repo} is:open label:"${NEED_INFO_LABEL}" updated:>${twoDaysAgo.toISOString()}`, async (item) => {
    const { data: comments } = await github.rest.issues.listComments({
      owner, repo, issue_number: item.number, sort: 'created', direction: 'desc', per_page: 5
    });

    // Check if the last comment is from a non-maintainer
    const lastComment = comments[0];
    if (lastComment && !['OWNER', 'MEMBER', 'COLLABORATOR'].includes(lastComment.author_association) && lastComment.user.type !== 'Bot') {
      core.info(`Removing ${NEED_INFO_LABEL} from #${item.number} due to contributor response.`);
      if (!dryRun) {
        await github.rest.issues.removeLabel({ owner, repo, issue_number: item.number, name: NEED_INFO_LABEL }).catch(() => {});
      }
    }
  });

  // Closure: Check issues with the label that haven't been updated in 14 days
  await processItems(`repo:${owner}/${repo} is:open label:"${NEED_INFO_LABEL}" updated:<${noResponseThreshold.toISOString()}`, async (item) => {
  ...
  await processItems(`repo:${owner}/${repo} is:open -label:"${STALE_LABEL}" ${exemptQuery} updated:<${staleThreshold.toISOString()}`, async (item) => {
    core.info(`Marking #${item.number} as stale.`);
    if (!dryRun) {
      await github.rest.issues.addLabels({ owner, repo, issue_number: item.number, labels: [STALE_LABEL] });
      await github.rest.issues.createComment({
        owner, repo, issue_number: item.number,
  ...
  await processItems(`repo:${owner}/${repo} is:open label:"${STALE_LABEL}" updated:<${closeThreshold.toISOString()}`, async (item) => {
