...
Container-based [sandboxing](#sandboxing) is highly recommended and requires, at a minimum, setting `GEMINI_SANDBOX=true` in your `~/.env` and ensuring a container engine (e.g. `MacOS Seatbelt` or `docker` or `podman`) is available. See [Sandboxing](#sandboxing) for details.
