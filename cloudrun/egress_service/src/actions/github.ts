...
export async function handleEgressEvent(event: EgressEvent): Promise<void> {
  ...
  switch (action) {
    case 'COMMENT':
      ...
      console.log(`[EGRESS_GITHUB] Posting comment to ${owner}/${repo}#${issueNumber}...`);
    ...
    case 'LABEL':
      ...
      console.log(`[EGRESS_GITHUB] Adding labels [${payload.labels.join(', ')}] to ${owner}/${repo}#${issueNumber}...`);
