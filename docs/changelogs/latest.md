...
- **Terminal Performance and UI Polish:** Implemented "TerminalBuffer" mode to
  drastically reduce UI flicker, added a scrollbar for the input prompt, and
  refined tool output layouts for a cleaner display.
- **Enhanced Sandbox Reliability:** Significant improvements to Linux and
  Windows sandboxing, including better error matching, caching, and a critical
  fix for ARG_MAX crashes on Linux.
- **Background Process Monitoring:** Introduced new tools to monitor and inspect
  background processes, providing better visibility into long-running tasks.
- **Skill System and Subagent Enhancements:** Improved skill system instructions
  injection and added support for selective topic expansion and click-to-expand
  in the UI.
...
- refactor(cli): remove duplication in interactive shell awaiting input hint by
  @JayadityaGit in [#24801](https://github.com/JayadityaGit)
