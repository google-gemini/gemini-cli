...
- **Offline Search Support**: Bundled `ripgrep` binaries directly into the
  Single Executable Application (SEA) to provide robust, offline search
  capabilities without external dependencies.
- **MCP Resource Management**: Enhanced Model Context Protocol (MCP) integration
  with new tools for discovering, listing, and reading resources from MCP
  servers.
- **Prompt-Driven Memory System**: Replaced the standalone Memory Manager Agent
  with a more efficient and precise prompt-driven system for managing personal
  and project-scoped memory.
- **Local Model Streamlining**: Simplified the setup process for running
  `gemini gemma` local models, improving the experience for hybrid local/cloud
  workflows.
- **Narrative Flow Enhancements**: Promoted topic update narration to a general
  feature and enabled it by default to provide better context during
  long-running tasks.
- **Security & Stability**: Addressed critical security concerns regarding
  workspace `.env` overrides and implemented numerous stability fixes for SSL
  streaming, session persistence, and theme management.
