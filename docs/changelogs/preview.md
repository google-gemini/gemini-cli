...
- **Real-Time Voice Mode:** Implementation of a new real-time voice mode
  featuring both cloud and local backend support for interactive voice
  experiences.
- **Gemma 4 Model Support:** Experimental integration for the upcoming Gemma 4
  models, expanding the range of supported local and cloud-based LLMs.
- **Security & Workspace Trust:** Enhanced security measures for `.env` file
  loading and the enforcement of workspace trust requirements, particularly for
  headless environments.
- **Improved Core Architecture:** Significant internal refactoring with the
  introduction of a new `ContextManager` and `AgentChatHistory` for better
  session and state management.
- **Enhanced Safety & Validation:** Improved shell command validation, a new
  core tools allowlist, and robust handling for unhandled loop detection errors.
