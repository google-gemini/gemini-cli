...
In order to use this feature, the local Gemma model **must** be served behind a Gemini API and
accessible via HTTP at an endpoint configured in `settings.json`.

For more details on how to configure local model routing, see [Local Model Routing](../core/local-model-routing.md).
...
3.  **`model.name` in `settings.json`:** If neither of the above are set, the
    ...
    will be used.
4.  **Local model (experimental):** If a local model router is configured in your `settings.json`
    file, the CLI will use the local Gemma model as configured.
