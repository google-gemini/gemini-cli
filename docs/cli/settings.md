...
| Plan Directory          | `general.plan.directory`           | The directory where planning artifacts are stored. If not specified, Gemini CLI uses its managed [temporary directory](../reference/configuration.md#temporary-directory). A custom directory requires a policy to allow write access in Plan Mode.           | `undefined` |
