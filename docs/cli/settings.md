...
| Force Alternate Buffer               | `ui.compatibility.forceAltBuffer`        | Force enable the alternate screen buffer even in potentially buggy terminals. Can also be controlled via the `GEMINI_CLI_FORCE_ALT_BUFFER=1` environment variable. | `false`  |
| Disable Alternate Buffer             | `ui.compatibility.disableAltBuffer`      | Disable the alternate screen buffer globally. Can also be controlled via the `GEMINI_CLI_DISABLE_ALT_BUFFER=1` environment variable.                               | `false`  |
| Disable Mouse Support                | `ui.compatibility.disableMouse`          | Disable mouse event tracking. Can also be controlled via the `GEMINI_CLI_DISABLE_MOUSE=1` environment variable.                                                    | `false`  |
| Assume Trusted Terminal              | `ui.compatibility.assumeTrustedTerminal` | Assume the terminal is fully capable and skip capability fallbacks. Can also be controlled via the `GEMINI_CLI_ASSUME_TRUSTED_TERMINAL=1` environment variable.    | `false`  |
