...
| Hide Context Window Percentage | `ui.footer.hideContextPercentage`        | Hides the context window remaining percentage. Note: This setting only takes effect when both `ui.footer.hideModelInfo` and `ui.hideFooter` are `false`. | `true`  |
