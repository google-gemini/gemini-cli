...
Run Gemini CLI in headless mode by providing the `-p` or `--prompt` flag
followed by your prompt. This bypasses the interactive chat interface and prints
the response to standard output (stdout).
