...
Run Gemini CLI in headless mode by providing a prompt using the `-p` or
...
```bash
gemini --prompt "Write a poem about TypeScript"
```
...
```bash
cat error.log | gemini --prompt "Explain why this failed"
```

**Windows (PowerShell)**

```powershell
Get-Content error.log | gemini --prompt "Explain why this failed"
```

Pipe a command:

```bash
git diff | gemini --prompt "Write a commit message for these changes"
```
