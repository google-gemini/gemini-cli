...
- **Agent delegation** (using tools like `docs-writer` or `generalist`) defaults
  to **`ask_user`** to ensure remote agents can prompt for confirmation, but
  local sub-agent actions are executed silently and checked individually.
