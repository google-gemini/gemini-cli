...
Each sub-agent has a maximum number of turns (`max_turns`) and a time limit
(`timeout_mins`). When a sub-agent exceeds these limits, you will see an
...
Replace `agent` with the name of the agent you want to
configure. You can override one or both values. Settings overrides apply on top
of the agent's definition, so you only need to specify the values you want to
change.
