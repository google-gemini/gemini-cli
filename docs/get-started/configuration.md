...
- **Properties:**
  ...
  - **`otlpProtocol`** (string): The protocol for the OTLP Exporter (`grpc` or
    `http`).
  - **`otlpHeaders`** (object): Custom headers to send with OTLP telemetry requests.
    Useful for authentication (e.g., API keys, bearer tokens). Values can
...
>>>>>>> a0893801 (cleanup(markdown): Prettier format all markdown @ 80 char width (#10714))
