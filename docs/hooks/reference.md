...
- **Input Fields**:
  ...
  - `tool_input`: (`object`) The arguments passed to the tool. This is usually
    the raw model-generated input, but some tools may include trusted
    runtime-derived fields for hook use. For `exit_plan_mode`, hooks receive the
    raw `plan_filename` plus a resolved absolute `plan_path`.
...
- **Input Fields**:
  - `tool_name`: (`string`)
  - `tool_input`: (`object`) The tool arguments. This is usually the original
    input, but some tools may include trusted runtime-derived fields for hook
    use. For `exit_plan_mode`, hooks receive the raw `plan_filename` plus a
    resolved absolute `plan_path`.
