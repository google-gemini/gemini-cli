...
- **Input Fields**:
  ...
  - `agent_name`: (`string | undefined`) The name of the subagent (e.g.
    `"browser_agent"`, `"codebase_investigator"`). `undefined` for the main
    agent. Use this to skip hooks that should only run for the main agent.
- **Relevant Output Fields**:
  - `hookSpecificOutput.additionalContext`: Text that is **appended** to the
