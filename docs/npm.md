...
This monorepo contains two main packages: `@icarus603/gemini-code` and `@icarus603/gemini-code-core`.

## `@icarus603/gemini-code`
...
When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@icarus603/gemini-code-core`. This means that whether a user installs the package with `npm install -g @icarus603/gemini-code` or runs it directly with `npx @icarus603/gemini-code`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/Icarus603/gemini-code/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
Every night at midnight UTC, the [Release workflow](https://github.com/Icarus603/gemini-code/actions/workflows/release.yml) runs automatically on a schedule. It performs the following steps:
...
```bash
npm install -g @icarus603/gemini-code@nightly
```
...
After the workflow has successfully completed, you can monitor its progress in the [GitHub Actions tab](https://github.com/Icarus603/gemini-code/actions/workflows/release.yml). Once complete, you should:

1.  Go to the [pull requests page](https://github.com/Icarus603/gemini-code/pulls) of the repository.
...
- `npx -y @icarus603/gemini-code@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @icarus603/gemini-code@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @icarus603/gemini-code && npm uninstall -g @icarus603/gemini-code && npm cache clean --force &&  npm install @icarus603/gemini-code@<version>`
...
1.  Go to the [Actions tab](https://github.com/Icarus603/gemini-code/actions/workflows/release.yml) of the repository.
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @icarus603/gemini-code`.
