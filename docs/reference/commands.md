...
- **Description:** Clear the agent conversation context (active conversation history)
  and start a new session, in addition to clearing the visible terminal screen,
  history, and scrollback within the CLI.
- **Keyboard shortcut:** Press **Ctrl+L** at any time to perform a visual clear
  action (note that **Ctrl+L** only clears the terminal display and redraws the UI,
  preserving the active conversation context; use `/clear` to start a new session).
