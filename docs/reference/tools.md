...
function. Follow each tool link for full parameters, examples, and caveats.

| Category    | Tool                                             | Kind          | Description                                                                                                                                        |
| :---------- | :----------------------------------------------- | :------------ | :------------------------------------------------------------------------------------------------------------------------------------------------- |
| Execution   | [`run_shell_command`](../tools/shell.md)         | `Execute`     | Executes shell commands, including interactive sessions and background processes. Requires manual confirmation.                                    |
| File System | [`glob`](../tools/file-system.md)                | `Search`      | Finds files matching specific glob patterns across the workspace.                                                                                  |
| File System | [`grep_search`](../tools/file-system.md)         | `Search`      | Searches file contents for a regular expression pattern. Legacy alias: `search_file_content`.                                                      |
| File System | [`list_directory`](../tools/file-system.md)      | `Read`        | Lists the names of files and subdirectories within a specified path.                                                                               |
| File System | [`read_file`](../tools/file-system.md)           | `Read`        | Reads the content of a specific file, including text, images, audio, and PDFs.                                                                     |
...
| Interaction | [`write_todos`](../tools/todos.md)               | `Other`       | Maintains an internal task list so the model can track and display its own progress.                                                               |
...
| Memory      | [`get_internal_docs`](../tools/internal-docs.md) | `Think`       | Accesses Gemini CLI's own documentation to answer questions about its capabilities more accurately.                                                |
...
| Planning    | [`enter_plan_mode`](../tools/planning.md)        | `Plan`        | Switches the CLI to a safe, read-only mode for researching complex changes.                                                                        |
...
| Web         | [`web_fetch`](../tools/web-fetch.md)             | `Fetch`       | Retrieves and processes content from specific URLs. This can reach local or private network addresses, so use it carefully with untrusted prompts. |
