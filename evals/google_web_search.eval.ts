...
import { validateModelOutput, printDebugInfo } from './test-helper.js';

describe('google_web_search', () => {
  evalTest('ALWAYS_PASSES', {
    name: 'should be able to search the web',
    params: {
      settings: { tools: { core: ['google_web_search'] } },
    },
      ...
      try {
        ...
        if (!foundToolCall) {
          ...
          const failedSearchCalls = allTools.filter(
            (t) =>
              t.toolRequest.name === 'google_web_search' &&
