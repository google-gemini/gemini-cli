...
1. target_files_score (0-2): Evaluate how well the candidate's target files match the Golden Spec:
   - Score 2 (Full Credit): The candidate accurately identifies all primary target files (or valid alternative target files in parenthetical format).
   - Score 1 (Partial Credit): The candidate correctly identifies at least one primary target file (or a closely related parent/child file in the same call chain), but misses some key files or includes extra non-essential files.
   - Score 0 (No Credit): The candidate completely misses all target files or only includes completely irrelevant files.
2. root_cause_and_summary_score (0-2): Does the candidate's problem statement and root cause analysis accurately identify the underlying defect or error? (Focus strictly on diagnostic accuracy independently of target files, not fix design or file path matching).
