...
# Broaden search to all open issues in the repo
...
def main():
    ...
    for issue in issues:
        ...
        for event in timeline_nodes:
            ...
            # 2. PR is linked (officially or unofficially)
            closing_issues = [n.get('number') for n in source.get('closingIssuesReferences', {}).get('nodes', [])]
            is_linked = (issue_number in closing_issues) or (f"#{issue_number}" in source.get('title', ''))
            # Let's stick to officially linked for high quality, or cross-referenced source.
            if not is_linked:
                # If it's a cross-reference, it's linked enough for triage
                pass 
    ...
    # Mentioned issues
    mentioned = [22929, 22920, 17235, 18727, 18751, 20675, 22591, 22616, 22351, 21635, 21370, 21036, 22321, 22611, 20500]
