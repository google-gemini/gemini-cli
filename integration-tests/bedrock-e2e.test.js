...
const shouldRunBedrockTests =
  process.env.BEDROCK_E2E_TESTS === 'true' &&
...
bedrockTest('reads file with AWS Bedrock', async (t) => {
  ...
  const output = rig.run(
    'read the file bedrock-test.txt and tell me what it contains',
  );
...
bedrockTest('writes file with AWS Bedrock', async (t) => {
  ...
  rig.run(
    'create a file called bedrock-output.txt with the message "Written by Claude via AWS Bedrock"',
  );
...
bedrockTest('handles JSON mode with AWS Bedrock', async (t) => {
  ...
  const output = rig.run(
    '--json generate a JSON object with status: "success" and provider: "bedrock"',
  );
  ...
  assert.ok(
    parsed.status === 'success' || parsed.provider === 'bedrock',
...
bedrockTest('handles multi-file operations with AWS Bedrock', async (t) => {
  ...
  rig.run(
    'read test1.js and test2.js, then create a summary.txt file listing what each file does',
  );
...
bedrockTest('uses grep functionality with AWS Bedrock', async (t) => {
  ...
  rig.createFile(
    'readme.md',
    'This project uses an API endpoint for communication.',
  );
...
bedrockTest('streams responses with AWS Bedrock', async (t) => {
  ...
  assert.ok(
    output.toLowerCase().includes('bedrock') ||
