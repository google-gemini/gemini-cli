  ...
  it('should perform real clipboard image operations', async () => {
    ...
    try {
      if (platform === 'darwin') {
        ...
        // Read the PNG file as image data and set clipboard to it
        await execFileAsync('osascript', [
          '-e',
          `set the clipboard to (read (POSIX file "${testImagePath}") as «class PNGf»)`,
        ]);
      ...
      // Test 1: Verify clipboard utility functions
...
Line 4: Unicode: 🚀🌟💻🔥✨`;
  ...
  it('should handle empty strings and whitespace content', async () => {
    ...
    // On Windows, clipboard may trim whitespace
