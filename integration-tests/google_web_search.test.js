...
test('should be able to search the web', async () => {
  ...
  const mentionsWeather = result.toLowerCase().includes('weather') || 
  ...
  if (!mentionsWeather || !mentionsLondon) {
    console.warn('Warning: LLM response may not include weather information or London.');
    console.warn(`Missing: ${!mentionsWeather ? 'weather info ' : ''}${!mentionsLondon ? 'London' : ''}`);
    console.warn('The google_web_search tool was called successfully, which is the main requirement.');
    
    // Log the search query used
    const searchCalls = rig.readToolLogs().filter(t => t.toolRequest.name === 'google_web_search');
    ...
    if (process.env.VERBOSE === 'true') {
      console.log('Web search completed successfully. Weather information for London provided.');
    }
  }
});
