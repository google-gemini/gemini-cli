...
// Helper to conditionally skip tests - bash scripts don't work on Windows
const itIfNotWindows = platform() !== 'win32' ? it : it.skip;
...
describe('Hooks System Integration', () => {
  ...
  describe('BeforeModel Hooks - LLM Request Modification', () => {
    // Uses bash script file - skip on Windows
    itIfNotWindows(
      'should modify LLM requests with BeforeModel hooks',
      async () => {
...
echo '{
  ...
  describe('BeforeAgent Hooks - Prompt Augmentation', () => {
    // Uses bash script file - skip on Windows
    itIfNotWindows(
      'should augment prompts with BeforeAgent hooks',
      async () => {
...
echo '{
  ...
  describe('Hook Input/Output Validation', () => {
    // Uses bash script file with jq - skip on Windows
    itIfNotWindows('should provide correct input format to hooks', async () => {
...
echo '{"decision": "allow", "hookSpecificOutput": {"hookEventName": "BeforeAgent", "additionalContext": "BeforeAgent: User request processed"}}'`;
  ...
  describe('Hook Error Handling', () => {
    // Uses bash script file - skip on Windows
    itIfNotWindows('should handle hook failures gracefully', async () => {
...
echo '{"decision": "allow", "reason": "Working hook succeeded"}'`;
  ...
  describe('Hook Telemetry and Observability', () => {
    // Uses bash script file - skip on Windows
    itIfNotWindows(
      'should generate telemetry events for hook executions',
      async () => {
        await rig.setup(
          'should generate telemetry events for hook executions',
          {
