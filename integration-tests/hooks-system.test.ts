  ...
  describe('Hooks "ask" Decision Integration', () => {
    it(
      ...
      async () => {
        ...
        const scriptPath = join(rig.testDir!, 'ask_hook.js');
        writeFileSync(scriptPath, hookScript);
        const commandPath = scriptPath.replace(/\\/g, '/');
                ...
                BeforeTool: [
                  {
                    ...
                    hooks: [
                      {
                        type: 'command',
                        command: `node "${commandPath}"`, // Corrected: escaped backslash in command path
                        timeout: 5000,
                      },
                    ],
                  },
    ...
    it('should allow cancelling when hook forces "ask" decision', async () => {
      ...
      const scriptPath = join(rig.testDir!, 'ask_cancel_hook.js');
      writeFileSync(scriptPath, hookScript);
      const commandPath = scriptPath.replace(/\\/g, '/');
