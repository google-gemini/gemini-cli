...
test('should be able to list a directory', async () => {
  ...
  if (!foundToolCall || !result.includes('file1.txt') || !result.includes('subdir')) {
    ...
    console.error('List directory calls:', allTools.filter(t => t.toolRequest.name === 'list_directory').map(t => t.toolRequest.args));
  }
  ...
  if (!hasFile1 || !hasSubdir) {
    console.warn('Warning: LLM did not include all files/directories in response.');
    console.warn(`Missing: ${!hasFile1 ? 'file1.txt ' : ''}${!hasSubdir ? 'subdir' : ''}`);
    console.warn('The list_directory tool was called successfully, which is the main requirement.');
  } else {
    // Log success info if verbose
    if (process.env.VERBOSE === 'true') {
      console.log('Directory listed successfully. Found file1.txt and subdir in output.');
    }
  }
});
