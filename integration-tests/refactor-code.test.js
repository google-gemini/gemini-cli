...
`;
  ...
  it('should extract a block of code into a new method', async () => {
    ...
    assert.strictEqual(
      result,
      `Successfully extracted code to new method 'calculateSum' in ${testFile}`,
    );
    ...
    assert.ok(
      newContent.includes('this.calculateSum();'),
      'The new method should be called.',
    );

    // Verify the new method exists
    assert.ok(
      newContent.includes('private calculateSum() {'),
      'The new method definition should exist.',
    );

    // Verify the extracted code is inside the new method
    assert.ok(
      newContent.includes('const z = x + y;'),
      'The extracted code should be in the new method.',
    );

    // Verify the original code is removed from the old method
    const oldMethodContent = newContent.match(
      /myMethod\(\) \{([\s\S]*?)\}/s,
    )[1];
    assert.ok(
      !oldMethodContent.includes('const z = x + y;'),
      'The extracted code should be removed from the original method.',
    );
  });
});
