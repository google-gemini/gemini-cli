...
export class ChatBridgeHandler {
  ...
  private async handleMessage(event: ChatEvent): Promise<ChatResponse> {
    ...
    if (trimmed === '/esc') {
      if (session.asyncProcessing) {
        ...
        if (session.taskId) {
          try {
            await this.a2aClient.cancelTask(session.taskId);
          } catch (err) {
            logger.warn(`[ChatBridge] cancelTask failed: ${err}`);
          }
        }
  ...
  private async processToolApprovalAsync(
    ...
    try {
      ...
      tracker.addToolActivity(
        approval.toolName,
        outcome === 'cancel' ? 'rejected' : 'approved',
      );
      ...
      // Push final text with activity card if we haven't already pushed a card
      if (lastText && !sentResponse) {
        if (tracker.hasActivity()) {
          const activityCard = renderActivityCard(tracker.getEntries());
          await this.chatApiClient.sendMessage(spaceName, threadName, {
  ...
  private async processMessageAsync(
    ...
    try {
      ...
      // Activity tracking for collapsible Cards V2
      const tracker = new ActivityTracker();
      let activityMessageName: string | undefined;
      let lastCardPushTime = 0;
      const CARD_PUSH_INTERVAL_MS = 60_000; // Push activity card at most once per minute
      ...
      // Track tools across events: key → {label, trackerIndex, surfaceId}
      // Supports status updates when the same tool appears in later events.
      const toolTrackingMap = new Map<
        string,
        { label: string; trackerIndex: number; surfaceId: string }
      >();
      // Snapshot of lastText when we last saw a tool — used to extract
      // narration text between tool calls for the activity card.
      let lastTextSnapshotAtTool = '';
      ...
      for await (const streamEvent of stream) {
        ...
        logger.info(
          `[ChatBridge] Stream event #${eventCount}: kind=${streamEvent.kind}, ` +
            ...
            `tools=${extracted.toolApprovals.length} (pending=${extracted.toolApprovals.filter((a) => a.status === 'awaiting_approval').length})`,
        );
        ...
        // Track ALL tool surfaces in the activity card (including auto-approved in YOLO mode).
        // Uses a Map to update status when the same tool appears in later events
        // (e.g., awaiting_approval → success).
        for (const tool of extracted.toolApprovals) {
          const surfaceId = tool.callId.startsWith('tool_approval_')
            ? tool.callId // Orphaned status update — callId IS the surfaceId
            : `tool_approval_${tool.taskId}_${tool.callId}`;

          const existing = toolTrackingMap.get(surfaceId);
          if (existing) {
            // Update status of existing tool entry
            if (existing.label !== tool.status) {
              tracker.updateToolStatus(
                existing.trackerIndex,
                existing.label,
                tool.status,
              );
            }
            ...
            // This gives the card a natural "narration → tool → narration → tool" flow.
            if (lastText && lastText !== lastTextSnapshotAtTool) {
              const narration =
                lastTextSnapshotAtTool &&
                lastText.startsWith(lastTextSnapshotAtTool)
                  ? lastText.substring(lastTextSnapshotAtTool.length).trim()
                  : lastText.trim();
              if (narration) {
                tracker.addTextEntry(narration);
              }
            }
            lastTextSnapshotAtTool = lastText;

            // Build a descriptive label: "Shell: uname -a" instead of just "Shell"
            let label = tool.displayName || tool.name || 'Tool';
            if (tool.args) {
              try {
                const raw: unknown = JSON.parse(tool.args);
                if (
                  typeof raw === 'object' &&
                  raw !== null &&
                  !Array.isArray(raw)
                ) {
                  // Extract the most informative arg (command, path, query, etc.)
                  const rec = Object.fromEntries(Object.entries(raw));
                  const summary =
                    rec['command'] ||
                    rec['path'] ||
                    rec['file_path'] ||
                ...
                // args might not be valid JSON — use as-is if short enough
                if (tool.args.length <= 80) {
                  label = `${label}: ${tool.args}`;
                }
              }
            }
            const idx = tracker.addToolActivity(label, tool.status);
            toolTrackingMap.set(surfaceId, {
              label,
              trackerIndex: idx,
              surfaceId,
            });
        ...
        // Time-based activity card push (at most once per minute)
        if (
          tracker.hasActivity() &&
          Date.now() - lastCardPushTime >= CARD_PUSH_INTERVAL_MS
        ) {
          const card = renderActivityCard(tracker.getEntries());
          if (!activityMessageName) {
            activityMessageName = await this.chatApiClient.sendMessage(
              spaceName,
              threadName,
              { cardsV2: [card] },
            );
          } else {
            await this.chatApiClient.updateMessage(activityMessageName, {
              cardsV2: [card],
            });
          }
          lastCardPushTime = Date.now();
          logger.info(
            `[ChatBridge] Pushed activity card: ${tracker.count} entries`,
          );
        }
      ...
      logger.info(
        `[ChatBridge] Stream complete: ${eventCount} events, ` +
          ...
          `activity=${tracker.count} entries, ` +
      ...
      // Push final response if we haven't already pushed a tool approval
      if (lastText && !sentFinalResponse) {
        // Extract only the post-tool text (text generated after the last tool call).
        // The A2A protocol streams accumulated text, so if the full text starts
        // with the pre-tool text, the suffix is the actual final answer.
        let finalText = lastText;
        if (
          tracker.hasActivity() &&
          textBeforeTools &&
          lastText.startsWith(textBeforeTools)
        ) {
          const postToolText = lastText
            .substring(textBeforeTools.length)
            .trim();
          if (postToolText) {
            finalText = postToolText;
          }
          // If postToolText is empty, fall back to the full lastText
        }

        // Push final activity card (tools/thoughts only) if entries exist
        if (tracker.hasActivity()) {
          const finalCard = renderActivityCard(tracker.getEntries());
          if (activityMessageName) {
            await this.chatApiClient.updateMessage(activityMessageName, {
              cardsV2: [finalCard],
            });
          ...
          logger.info(
            `[ChatBridge] Final activity card: ${tracker.count} entries`,
          );
        }

        // Send final text as a separate message
        await this.chatApiClient.sendMessage(spaceName, threadName, {
          text: finalText,
        });
        logger.info(
          `[ChatBridge] Pushed final response (${finalText.length} chars, ` +
            `total=${lastText.length}, preTools=${textBeforeTools.length}): ` +
  ...
  private async autoApproveTools(
    ...
    for (let round = 0; round < MAX_ROUNDS && !session.cancelled; round++) {
      ...
      for (const a of approvalsToProcess) {
        ...
        tracker?.addToolActivity(label, 'auto-approved');
      }
