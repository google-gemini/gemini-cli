...
/**
 * Flag to indicate that the MCP serve command is running.
 * This is set synchronously in the handler to prevent main() from continuing.
 */
export let isMcpServeRunning = false;

/**
 * Command handler for `gemini mcp serve`
 *
 * Starts Gemini CLI as an MCP server, exposing Gemini capabilities
 * to external MCP clients via stdio transport.
 */
async function handleMcpServe(argv: ArgumentsCamelCase<ServeArgs>): Promise<void> {
  try {
    // Load settings
...
/**
 * The `gemini mcp serve` command module
 */
export const serveCommand: CommandModule<object, ServeArgs> = {
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .example('$0 mcp serve', 'Start Gemini as an MCP server')
      .example(
        'GEMINI_API_KEY=xxx $0 mcp serve',
        'Start with API key authentication',
      )
      .example(
        '$0 mcp serve --model gemini-2.0-flash',
