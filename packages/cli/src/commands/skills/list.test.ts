...
describe('skills list command', () => {
  ...
  describe('handleList', () => {
    ...
    it('should list all discovered skills', async () => {
      ...
      expect(emitConsoleLog).toHaveBeenCalledWith('log', 'Discovered Skills:');
      expect(emitConsoleLog).toHaveBeenCalledWith('log', '- skill1 [Enabled]');
      expect(emitConsoleLog).toHaveBeenCalledWith('log', '- skill2 [Disabled]');
    });
