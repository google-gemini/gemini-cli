...
describe('loadCliConfig autoMemory compatibility', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
    vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
  ...
  it('should preserve legacy auto memory behavior when only memoryManager is enabled', async () => {
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
      experimental: {
        memoryManager: true,
    ...
    delete settings.experimental.autoMemory;
    const config = await loadCliConfig(
      settings,
      'test-session',
      argv,
  ...
  it('should let explicit autoMemory false override legacy memoryManager compatibility', async () => {
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings({
        experimental: {
