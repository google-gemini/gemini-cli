...
describe('Background completion behavior', () => {
  beforeEach(() => {
    vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([]);
  });
  ...
  it('should default to silent', async () => {
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
    ...
    expect(config.getShellBackgroundCompletionBehavior()).toBe('silent');
  });

  it('should use the behavior from settings', async () => {
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings({
    ...
    expect(config.getShellBackgroundCompletionBehavior()).toBe('inject');
  });

  it('should prioritize the behavior from argv', async () => {
    process.argv = [
      'node',
      'script.js',
  ...
  it('should reject invalid --background-completion-behavior argument', async () => {
    process.argv = [
      'node',
      'script.js',
      '--background-completion-behavior',
