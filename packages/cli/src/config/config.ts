...
export async function loadCliConfig(
  ...
  if (rawApprovalMode) {
    switch (rawApprovalMode) {
      ...
      case 'plan':
        if (
          !(
            settings.general?.plan?.enabled ??
            (settings.experimental as Record<string, unknown>)?.['plan']
          )
        ) {
          approvalMode = ApprovalMode.DEFAULT;
        } else {
  ...
  if (settings.security?.disableYoloMode || settings.admin?.secureModeEnabled) {
    if (isYoloRequested || allowedTools.includes('*')) {
      const modeName = settings.admin?.secureModeEnabled
        ? 'secureModeEnabled'
        : 'disableYoloMode';
      debugLogger.error(
        `YOLO mode (wildcard policies) are disabled by ${modeName} setting.`,
      );

      isYoloRequested = false;
  ...
  if (!interactive || !!argv.acp || !!argv.experimentalAcp) {
