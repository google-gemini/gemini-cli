...
describe('Settings Loading and Merging', () => {
  ...
  describe('Advanced Environment Variable Resolution', () => {
    it('should handle environment variables with special characters', () => {
      ...
      expect(result.user.settings.testValue).toBe('value with spaces and $pecial ch@rs & symbols!');
      expect(result.user.settings.jsonValue).toBe('{"key": "value", "array": [1,2,3]}');
      expect(result.user.settings.combined).toBe('prefix_value with spaces and $pecial ch@rs & symbols!_suffix');
    ...
    it('should handle environment variables with empty and whitespace values', () => {
      ...
      expect(result.user.settings.prefixedWhitespace).toBe('prefix_   \t\n  _suffix');
    ...
    it('should handle malformed environment variable syntax gracefully', () => {
      ...
      expect(result.user.settings.multipleIncomplete).toBe('${ONE ${TWO} ${THREE');
    ...
    it('should handle environment variables in different data structures', () => {
      ...
      const result = loadSettings(MOCK_WORKSPACE_DIR);
      expect(result.user.settings.array).toEqual(['string_value', 'literal', '42', 'true']);
  ...
  describe('Unicode and International Character Support', () => {
    ...
    it('should handle Unicode in environment variables', () => {
      ...
      const result = loadSettings(MOCK_WORKSPACE_DIR);
      expect(result.user.settings.unicodeFromEnv).toBe('🌟 Unicode value 世界 тест العربية');
      expect(result.user.settings.emojiFromEnv).toBe('🚀🎉✨');
      expect(result.user.settings.combined).toBe('prefix_🌟 Unicode value 世界 тест العربية_🚀🎉✨_suffix');
