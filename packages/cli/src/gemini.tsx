 ...
 */
function getThemePreferences(settings: LoadedSettings) {
  return {
...
async function initializeTheme(
  ...
  // If no theme is set, apply auto theme to match terminal background
  // But don't save it to settings - just apply it temporarily
  if (!themeName) {
    applyAutoTheme(terminalBackground, themePreferences, isDebugMode);
    // Check if we should prompt the user to enable auto theme permanently
    // (This only happens if there's a mismatch, but with no theme set, we've already
    // applied the correct theme, so we need to check what the default would have been)
    const defaultTheme = themeManager.findThemeByName('Default');
    if (
      defaultTheme &&
      hasThemeMismatch(terminalBackground, defaultTheme.type)
    ) {
      await handleThemeMismatch(terminalBackground, themePreferences, settings);
    }
