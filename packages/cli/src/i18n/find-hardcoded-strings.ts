...
class StringFinder {
  ...
  private readonly HIGH_PRIORITY_PATTERNS = [
    ...
    { pattern: /(?:text|title|label|placeholder):\s*["']([A-Z][^"']{3,})["']/g, category: 'UI Components' },
    // Directly rendered text
    { pattern: /<Text[^>]*>([A-Z][^<]{5,})<\/Text>/g, category: 'Direct Text' },
    // Dialogs and prompts
    { pattern: /["']([A-Z][a-z]+.*?(?:dialog|Dialog|confirm|Confirm|prompt|Prompt)[^"']{0,50})["']/g, category: 'Dialogs' },
    // Error and success messages
    { pattern: /["']((?:Error|Success|Failed|Successfully)[^"']{5,})["']/g, category: 'Status Messages' },
  ];
  ...
  private readonly MEDIUM_PRIORITY_PATTERNS = [
    ...
    { pattern: /description:\s*["']([A-Z][^"']{10,})["']/g, category: 'Command Descriptions' },
    // Text in addItem calls
    { pattern: /addItem\([^)]*text:\s*["']([A-Z][^"']{5,})["']/g, category: 'Message Items' },
    // Status and notification messages
    { pattern: /["']((?:Loading|Connecting|Connected|Disconnected|Installing|Restarting)[^"']{3,})["']/g, category: 'Status Updates' },
    // Validation and configuration messages
    { pattern: /["']((?:Please|Already|Cannot|Unable|Invalid)[^"']{10,})["']/g, category: 'Validation Messages' },
  ];

  // Low priority patterns - debug and internal text
  private readonly LOW_PRIORITY_PATTERNS = [
    // Debug and log messages
    { pattern: /["']((?:DEBUG|INFO|WARN)[^"']{10,})["']/g, category: 'Debug Messages' },
    // Developer related text
    { pattern: /["']([A-Z][a-z]+.*(?:development|Development|test|Test)[^"']{0,30})["']/g, category: 'Development' },
  ...
  isToolApiDescription(filePath: string, extractedText: string, context: string): boolean {
    ...
    if (filePath.includes('/telemetry/metrics.ts') && context.includes('description:')) {
      return true;
    }
    
    // Exclude AI tool API descriptions in subagent.ts
    if (filePath.includes('/core/subagent.ts') && context.includes('description:')) {
      return true;
    }
    
    // Descriptions in tool files are usually API documentation, not for end users
    if (filePath.includes('/tools/') && extractedText.match(/^(The|Optional|Whether|Performs|List|Reads|Writes|Counts)/)) {
      return true;
    }
    
    // Descriptions containing technical terms
    if (extractedText.match(/\b(API|JSON|regex|glob|pattern|endpoint|parameter|argument|metadata|telemetry)\b/i)) {
      return true;
    }
    ...
    lines.forEach((line, index) => {
      ...
      this.HIGH_PRIORITY_PATTERNS.forEach(({ pattern, category }) => {
        ...
        while ((match = regex.exec(line)) !== null) {
          ...
          if (extractedText && extractedText.length > 3 && !this.isToolApiDescription(filePath, extractedText, line)) {
            this.results.push({
              file: filePath,
              line: index + 1,
      ...
      this.MEDIUM_PRIORITY_PATTERNS.forEach(({ pattern, category }) => {
        ...
        while ((match = regex.exec(line)) !== null) {
          const extractedText = match[1];
          if (extractedText && extractedText.length > 5 && !this.isToolApiDescription(filePath, extractedText, line) && !this.isPartOfMultilineTranslatedCall(lines, index) && !this.isPartOfLegacyBackupCode(lines, index) && !this.isInTryCatchFallback(lines, index) && !this.isInKnownFallbackFile(filePath)) {
            this.results.push({
              file: filePath,
              line: index + 1,
              content: extractedText,
      ...
      this.LOW_PRIORITY_PATTERNS.forEach(({ pattern, category }) => {
        ...
        while ((match = regex.exec(line)) !== null) {
          const extractedText = match[1];
          if (extractedText && extractedText.length > 15 && !this.isToolApiDescription(filePath, extractedText, line) && !this.isPartOfMultilineTranslatedCall(lines, index) && !this.isPartOfLegacyBackupCode(lines, index) && !this.isInTryCatchFallback(lines, index) && !this.isInKnownFallbackFile(filePath)) {
            this.results.push({
              file: filePath,
              line: index + 1,
              content: extractedText,
  ...
  private isPartOfMultilineTranslatedCall(lines: string[], currentIndex: number): boolean {
    ...
    for (let i = 0; i <= lookBackLines; i++) {
      ...
      if (line.includes('getTranslatedErrorMessage')) {
        ...
        const fromCallLine = lines.slice(lineIndex, currentIndex + 1).join('\n');
  ...
  private isInTryCatchFallback(lines: string[], currentIndex: number): boolean {
    ...
    for (let i = 0; i <= lookBackLines; i++) {
      ...
      if (line.includes('} catch') || line.includes('catch {') || line.includes('catch (')) {
  ...
  private isInKnownFallbackFile(filePath: string): boolean {
    ...
    return filePath.includes('usePhraseCycler.ts') && 
           filePath.includes('ui/hooks');
  }

  private isPartOfLegacyBackupCode(lines: string[], currentIndex: number): boolean {
  ...
  generateReport(): string {
    ...
    const groupedResults = priorityOrder.reduce((acc, priority) => {
      acc[priority] = this.results.filter(item => item.priority === priority);
      return acc;
    ...
    priorityOrder.forEach(priority => {
      ...
      const priorityName = priority === 'high' ? 'High Priority' : priority === 'medium' ? 'Medium Priority' : 'Low Priority';
    ...
    report += `\nBy category:\n`;
    categories.forEach(category => {
      const count = this.results.filter(item => item.category === category).length;
  ...
  async run(): Promise<void> {
    console.log('🔍 Searching for hardcoded strings that need internationalization...\n');
    ...
    console.log(`✨ After filtering internationalized: ${this.results.length} strings pending`);
    ...
    const highPriority = this.results.filter(r => r.priority === 'high').length;
    const mediumPriority = this.results.filter(r => r.priority === 'medium').length;
