...
export const zhMessages: I18nMessages = {
  ui: {
    auth: {
      ...
      vertexAi: 'Vertex AI',
      useCloudShell: '使用Cloud Shell用户凭据',
      ...
      success: '认证成功',
      failed: '认证失败',
      invalidDefaultAuthType: '无效的GEMINI_DEFAULT_AUTH_TYPE值："{{value}}"。有效值为：{{validValues}}。',
      existingApiKeyDetected: '检测到现有API密钥（GEMINI_API_KEY）。选择"Gemini API密钥"选项以使用它。',
      mustSelectAuthMethod: '您必须选择认证方式才能继续。按Ctrl+C两次退出。',
    },
    tips: {
      gettingStarted: '入门指南：',
