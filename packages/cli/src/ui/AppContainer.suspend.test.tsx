...
vi.mock('./hooks/useMcpStatus.js', () => ({
  useMcpStatus: () => ({ isMcpReady: true }),
}));
vi.mock('./hooks/useBackgroundShellManager.js', () => ({
...
describe('AppContainer Suspend (Ctrl+Z)', () => {
  ...
  it.skip('should suspend process on double Ctrl+Z and restore on resume', async () => {
    ...
    // Verify NO suspend actions
    expect(process.kill).not.toHaveBeenCalled();
    expect(writeToStdout).not.toHaveBeenCalledWith('\x1b[?25h');

    // Trigger Ctrl+Z Second Time
    await act(async () => {
      capturedHandler!({
        name: 'z',
        ctrl: true,
    ...
    // Verify suspend actions with waitFor
    await waitFor(() => {
      // 1. Show cursor
