...
describe('forkCommand', () => {
  ...
  const getSubCommand = (name: 'list' | 'save' | 'resume'): SlashCommand => {
    const subCommand = forkCommand.subCommands?.find(
      (cmd) => cmd.name === name,
    );
    if (!subCommand) {
      throw new Error(`/fork ${name} command not found.`);
    }
    return subCommand;
  };
  ...
  it('should have the correct main command definition', () => {
    ...
    expect(forkCommand.description).toBe(
      'Fork and manage conversation checkpoints.',
    );
    expect(forkCommand.subCommands).toHaveLength(3);
  });
  ...
  describe('save subcommand', () => {
    ...
    it('should return an error if tag is missing', async () => {
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'error',
        content: 'Missing tag. Usage: /fork save <tag>',
      });
    });
  ...
  describe('resume subcommand', () => {
    ...
    it('should return an error if tag is missing', async () => {
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'error',
        content: 'Missing tag. Usage: /fork resume <tag>',
      });
    });
