    ...
    action: async (context) => {
      ...
      if (!installer) {
        context.ui.addItem(
          {
            type: 'error',
            text: `No installer is available for ${ideClient.getDetectedIdeDisplayName()}. Please install the 'Gemini CLI Companion' extension manually from the marketplace. `,
          },
          Date.now(),
        );
        return;
