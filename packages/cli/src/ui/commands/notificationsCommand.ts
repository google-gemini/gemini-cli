...
import { triggerNotification, getNotificationSettings, setGlobalNotificationsEnabled, testNotifications, updateNotificationEventSettings } from '../../notifications/manager.js';
...
const disableCommand: SlashCommand = {
  ...
  action: async (context: CommandContext, args: string): Promise<SlashCommandActionReturn> => {
    ...
    await setGlobalNotificationsEnabled(false, context.services.config);
...
const statusCommand: SlashCommand = {
  ...
  action: async (context: CommandContext, args: string): Promise<SlashCommandActionReturn> => {
    ...
    for (const key in settings.events) {
      ...
      statusMessage += `        Command: ${eventSettings.command}\n`;
      if (eventSettings.customPath) {
