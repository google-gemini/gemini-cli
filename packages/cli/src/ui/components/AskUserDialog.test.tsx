...
describe('AskUserDialog', () => {
  ...
  it('allows question to exceed 15 lines in non-alternate buffer mode if space allows', () => {
    // 20 lines of question text
    ...
    const mockUIState = {
      availableTerminalHeight: 40, // Enough space for 20 lines + overhead
    ...
    // Should NOT contain the truncation message for the question
    // (Wait, since we can't easily check actual rendered lines in this test setup,
    // we check that 'Line 1' and 'Line 20' are both visible)
    const frame = lastFrame();
  ...
  it('allows more list items to be visible when the question is short', () => {
    const questions: Question[] = [
      {
        question: 'Short?',
        header: 'Short Q Test',
    ...
    // With height 30 and a short question, we should be able to show many items.
    // Overhead: ProgressHeader(2) + Title(1) + Footer(2) + dialogPadding(4) = 9.
    // listHeight = 30 - 5 = 25.
    // questionHeight (max) = 25 - 4 = 21.
    // If measuredHeight is 0 (since ResizeObserver doesn't run in test easily),
    // maxItemsToShow = (25 - 0) / 2 = 12.
    // So Option 12 should be visible.
    // (In the previous code, questionHeight was min(15, 21) = 15.
    // maxItemsToShow = (25 - 15) / 2 = 5. Option 12 would NOT be visible.)

    expect(lastFrame()).toContain('Option 12');
  });
