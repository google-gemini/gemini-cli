...
export const Footer: React.FC = () => {
  ...
  if (showCostEstimate) {
    const promptTokens = uiState.sessionStats.lastPromptTokenCount || 0;
    const currentModel = uiState.currentModel || 'gemini-2.5-flash';
    const estimatedCost = calculateCost(currentModel, promptTokens, 0);
    const formattedCost = formatCostEstimate(estimatedCost);

    addCol(
      'costEstimate',
      '[Cost Est]',
      () => <Text color={theme.text.primary}>{formattedCost}</Text>,
      formattedCost.length, // Let the new system calculate width automatically!
      false
    );
  }
