...
describe('handleAtCommand', () => {
  ...
  it('should process a valid directory path and convert to glob', async () => {
    ...
    const query = `@${dirPath}`;
    const resolvedGlob = `${dirPath}${path.sep}**`;
  ...
  it('should process a file path case-insensitively', async () => {
    ...
    await createTestFile(
      path.join(testRootDir, 'path', 'to', 'MyFile.txt'),
      fileContent,
    );
    // Use a query with a different casing.
    const queryPath = path.join(testRootDir, 'path', 'to', 'myfile.txt');
  ...
  describe('git-aware filtering', () => {
    ...
    beforeEach(async () => {
      await fsPromises.mkdir(path.join(testRootDir, '.git',), { recursive: true });
    });
