...
export function useCompletion(
  ...
  const pendingSearchPrefixRef = useRef<string | null>(null);
  ...
  const fetchSuggestions = useCallback(
    async (prefix: string, signal: AbortSignal) => {
      ...
      try {
        const results = await fileSearchRef.current!.search(prefix, signal);
        const truncatedResults = results.slice(0, MAX_SUGGESTIONS_TO_SHOW);

        const fetchedSuggestions = truncatedResults.map((result) => ({
          label: result,
  ...
  useEffect(() => {
    if (
      filesearchStatus === FileSearchStatus.READY &&
      pendingSearchPrefixRef.current
    ) {
      ...
      pendingSearchPrefixRef.current = null;
    }
  }, [filesearchStatus, fetchSuggestions]);
  ...
  useEffect(() => {
    ...
    if (filesearchStatus === FileSearchStatus.INITIALIZING) {
      pendingSearchPrefixRef.current = prefix;
      return;
    }
