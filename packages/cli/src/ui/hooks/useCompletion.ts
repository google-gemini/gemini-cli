...
export function useCompletion(
  ...
  const fetchFileSuggestions = useCallback(
    async (prefix: string, signal: AbortSignal) => {
      ...
      // Only show loading spinner if there are no suggestions currently displayed.
      // This prevents the spinner from flashing during rapid typing.
  ...
  useEffect(() => {
    if (isActive && fileSearchStatus === FileSearchStatus.IDLE) {
      ...
      fileSearchRef.current = new FileSearch({
        projectRoot: cwd,
        // TODO(b/343593467): Make these configurable.
        ignoreDirs: [],
        useGitignore: true,
  ...
  }, [isActive, fileSearchStatus, cwd, buffer.text]);
