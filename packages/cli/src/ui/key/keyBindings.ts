...
/**
 * Builds the default key binding configuration with platform-specific overrides.
 * - On Windows, Ctrl+Z is Undo (Win+Z opens Snap Layouts, and suspend is unsupported).
 * - On macOS/Linux, Cmd/Super+Z is Undo, Ctrl+Z is Suspend.
 */
function buildDefaultKeyBindingConfig(): KeyBindingConfig {
  ...
  return new Map([
    ...
    [
      ...
      [new KeyBinding('tab'), new KeyBinding('enter')],
    ],
    [Command.COMPLETION_UP, [new KeyBinding('up'), new KeyBinding('ctrl+p')]],
    [
      Command.COMPLETION_DOWN,
      [new KeyBinding('down'), new KeyBinding('ctrl+n')],
    ],
    ...
    [Command.SUSPEND_APP, isWindows ? [] : [new KeyBinding('ctrl+z')]],
 ...
 * Platform-aware: uses Ctrl+Z for Undo on Windows (Win+Z opens Snap Layouts),
 * and Cmd/Super+Z on macOS/Linux.
 */
export const defaultKeyBindingConfig: KeyBindingConfig =
  buildDefaultKeyBindingConfig();
