...
/**
 * Check if the current platform supports clipboard operations for testing
 */
async function checkClipboardSupport(): Promise<boolean> {
...
describeE2E('Clipboard E2E Tests', () => {
  ...
  it('should perform real clipboard image operations', async () => {
    ...
    // Check if this platform supports clipboard operations
    const supportsClipboard = await checkClipboardSupport();
    if (!supportsClipboard) {
      console.log(
        `Skipping clipboard test on ${platform} - clipboard tools not available`,
      );
      return;
    }
    ...
    try {
      ...
      const { keyMatchers, Command } = await import('../keyMatchers.js');
