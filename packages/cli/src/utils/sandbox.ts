...
export async function start_sandbox(
  ...
  const patcher = new ConsolePatcher({
    debugMode: cliConfig?.getDebugMode() || isDebugEnabled(),
    stderr: true,
  });
  patcher.patch();
      ...
      for (const hostPath of config.allowedPaths) {
        if (hostPath && path.isAbsolute(hostPath) && fs.existsSync(hostPath)) {
          ...
          try {
            ...
            debugLogger.log(
              `mounted allowed path '${hostPath}' into LXC container as device '${allowedDeviceName}' (ro)`,
            );
          } catch (err) {
            debugLogger.warn(
...
async function pullImage(
  ...
  return new Promise((resolve) => {
    ...
    const onStdoutData = (data: Buffer) => {
      if (cliConfig?.getDebugMode() || isDebugEnabled()) {
