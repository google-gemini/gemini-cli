...
function parseSequenceDiagram(spec: string): {
  participants: string[];
  messages: SeqMessage[];
} {
...
function renderCompactSequenceDiagram(spec: string, termWidth = 100): string {
  ...
  for (const participant of participants) {
    lines.push(
      `   ${chalk.gray('•')} ${chalk.white(participant.slice(0, termWidth - 8))}`,
    );
  }
...
function renderSequenceDiagram(spec: string, termWidth = 100): string {
  ...
  const colWidth = Math.max(
    14,
    Math.floor((termWidth - 4) / participants.length),
  );
  const lines: string[] = [];
  ...
  const renderLifelines = () => {
    return participants
      .map(() => {
        ...
        return (
          ' '.repeat(mid) + chalk.gray(BOX.V) + ' '.repeat(colWidth - mid - 1)
        );
      })
      .join('  ');
  };
    ...
    const rowParts = participants.map((_, i) => {
      ...
      if (i === fromIdx) {
        if (isLeft) {
          ...
          return (
            chalk.yellow(BOX.H.repeat(mid)) +
            chalk.yellow(BOX.H.repeat(colWidth - mid))
          );
        }
      }
      if (i === toIdx) {
...
function parseFlowEdge(line: string): FlowEdge | null {
  const labeledBarMatch = line.match(
    /^([\w:-]+)\s*-->\s*\|(.+?)\|\s*([\w:-]+)$/,
  );
  if (labeledBarMatch) {
    return {
  ...
  const labeledTextMatch = line.match(
    /^([\w:-]+)\s*--\s*(.+?)\s*-->\s*([\w:-]+)$/,
  );
  if (labeledTextMatch) {
    return {
...
function parseFlowchart(spec: string): {
  nodes: FlowNode[];
  edges: FlowEdge[];
} {
  const lines = spec
  ...
  for (const rawLine of lines) {
    ...
    if (
      nMatch &&
      nMatch[1] &&
      !['graph', 'flowchart', 'TD', 'LR', 'BT', 'RL'].includes(nMatch[1])
    ) {
...
function renderFlowchart(spec: string, termWidth = 100): string {
  ...
  for (let i = 0; i < sortedNodes.length; i++) {
    ...
    if (node.shape === 'diamond') {
      ...
      lines.push(
        indent + chalk.blue(`${BOX.TL}${BOX.H.repeat(boxWidth)}${BOX.TR}`),
      );
      lines.push(
        indent +
          chalk.blue(`${BOX.V}`) +
      ...
      lines.push(
        indent + chalk.blue(`${BOX.BL}${BOX.H.repeat(boxWidth)}${BOX.BR}`),
      );
    }
      ...
      if (nextNode && t.to === nextNode.id) {
        ...
        lines.push(
          indent +
            ' '.repeat(halfBox + 1) +
            chalk.gray('└─') +
            chalk.yellow(BOX.ARROW_R) +
            labelStr +
            chalk.bold.white(t.to),
        );
      }
    }
...
function parseStateDiagram(spec: string): {
  states: StateNode[];
  ...
  for (const line of lines) {
    ...
    if (tMatch) {
      ...
      if (from !== '[*]' && !states.has(from))
        states.set(from, { id: from, label: from, isStartEnd: false });
      if (to !== '[*]' && !states.has(to))
        states.set(to, { id: to, label: to, isStartEnd: false });
      continue;
    }
...
function renderPieChart(spec: string, termWidth = 100): string {
  ...
  for (const item of data) {
    ...
    const bar =
      chalk.green('█'.repeat(barLen)) + chalk.gray('░'.repeat(barMax - barLen));
    const label = item.label.slice(0, 15).padEnd(15);
    result.push(
      `  ${chalk.white(label)} | ${bar} ${chalk.bold.cyan(percent.toFixed(1) + '%')}`,
    );
  }
...
function parseErDiagram(spec: string): {
  entities: ErEntity[];
  ...
  for (const line of lines) {
    ...
    if (relMatch) {
      ...
      if (!entities.has(relMatch[1]))
        entities.set(relMatch[1], { name: relMatch[1], attributes: [] });
      if (!entities.has(relMatch[3]))
        entities.set(relMatch[3], { name: relMatch[3], attributes: [] });
      continue;
    }
...
function renderErDiagram(spec: string, termWidth = 100): string {
  ...
  for (const entity of entities) {
    const maxWidth =
      Math.max(entity.name.length, ...entity.attributes.map((a) => a.length)) +
      4;
    ...
    lines.push(
      `  ` +
        chalk.green(`${BOX.V}`) +
        chalk.bold.white(` ${entity.name.padEnd(maxWidth - 2)} `) +
        chalk.green(`${BOX.V}`),
    );
    ...
    for (const attr of entity.attributes) {
      lines.push(
        `  ` +
          chalk.green(`${BOX.V}`) +
          chalk.gray(` ${attr.padEnd(maxWidth - 2)} `) +
          chalk.green(`${BOX.V}`),
      );
    }
    ...
    for (const rel of entityRels) {
      ...
      lines.push(
        `       ` +
          chalk.yellow(`${rel.relationship}`) +
          chalk.gray(`${label}`) +
          chalk.bold.white(`${rel.entity2}`),
      );
    }
    lines.push('');
  }
...
function renderGenericFallback(spec: string, termWidth = 100): string {
  ...
  const lines = [
    ...
    `  ` +
      chalk.red(`│`) +
      chalk.bold.red(` ⚠ ASCII fallback — diagram source:`) +
      chalk.red(` │`),
