    ...
    systemPrompt:
      ...
      (config.isAgentsEnabled()
        ? '### Sub-Agents (Local & Remote)\n' +
          'Sub-agents are defined in `.gemini/agents/` or `~/.gemini/agents/` using TOML. Always reference the types and properties outlined here directly when answering questions about sub-agents.\n' +
