...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private async executeTurn(
    ...
    debugLogger.log(
      `[LocalAgentExecutor] Turn ${turnCounter} results: taskCompleted=${taskCompleted}, aborted=${aborted}, hasSubmittedOutput=${!!submittedOutput}`,
    );
  ...
  private async processFunctionCalls(
    ...
    for (const [index, functionCall] of functionCalls.entries()) {
      ...
      if (toolName === TASK_COMPLETE_TOOL_NAME && !this.usesCustomCompletionTool()) {
        if (taskCompleted) {
          const error =
