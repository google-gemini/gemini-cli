...
describe('oauth2', () => {
  ...
  it('should fall back to user code flow if browser launch fails', async () => {
    const mockAuthUrl = 'https://example.com/auth';
    const mockTokens = {
      access_token: 'test-access-token',
      refresh_token: 'test-refresh-token',
    ...
    // Mock the web auth flow to fail
    (open as Mock).mockRejectedValue(new Error('Failed to open browser'));
    ...
    // Verify that the web auth was attempted and failed
    expect(open).toHaveBeenCalledWith(mockAuthUrl);
    expect(mockHttpServer.close).toHaveBeenCalled();
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining('Falling back to user code flow'),
    );

    // Verify that the user code flow was executed
