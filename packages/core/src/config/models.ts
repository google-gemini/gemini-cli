 ...
 * @param useCustomToolModel Whether to use the custom tool model for auto/pro aliases.
...
export function supportsModernFeatures(
  ...
  if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
    return (
      config.modelConfigService.getModelDefinition(model)?.features?.modern ===
      true
    );
  }
...
export function isAutoModel(model: string, config?: Config): boolean {
  if (config?.getExperimentalDynamicModelConfiguration?.() === true) {
    return config.modelConfigService.getModelDefinition(model)?.isAuto === true;
  }
