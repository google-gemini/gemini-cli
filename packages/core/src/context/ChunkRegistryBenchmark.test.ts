...
describe('ChunkRegistry Performance Benchmarks', () => {
  ...
  it('should demonstrate O(1) addChunk performance', () => {
    ...
    for (const count of chunkCounts) {
      const chunks = Array.from({ length: count }, (_, i) => createTestChunk(`chunk-${i}`));
    ...
    console.log('Add Performance (ms):', timings.map((time, i) => 
      `${chunkCounts[i]} chunks: ${time.toFixed(2)}ms`
    ).join(', '));
  ...
  it('should demonstrate O(1) removeChunk performance with large registry', () => {
    ...
    const chunks = Array.from({ length: chunkCount }, (_, i) => createTestChunk(`chunk-${i}`));
    chunks.forEach(chunk => registry.addChunk(chunk));
    
    // Measure removal performance
    const removalTimings: number[] = [];
    const chunksToRemove = ['chunk-0', 'chunk-500', 'chunk-999', 'chunk-250', 'chunk-750'];
    ...
    const avgRemovalTime = removalTimings.reduce((sum, time) => sum + time, 0) / removalTimings.length;
    
    console.log(`Average removal time from ${chunkCount} chunks: ${avgRemovalTime.toFixed(4)}ms`);
  ...
  it('should demonstrate efficient chunk retrieval performance', () => {
    ...
    const operationNames = ['getChunk', 'getAllChunks', 'getChunksByRole', 'getTotalTokens', 'size'];
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      console.log(`${operationNames[i]} with ${chunkCount} chunks: ${duration.toFixed(4)}ms`);
  ...
  it('should demonstrate linear scaling for O(n) operations', () => {
    ...
    for (const size of sizes) {
      const registry = new ChunkRegistry();
      const chunks = Array.from({ length: size }, (_, i) => createTestChunk(`chunk-${i}`));
      chunks.forEach(chunk => registry.addChunk(chunk));
    ...
    console.log('Linear scaling (getAllChunks):', getAllChunksTimes.map((time, i) => 
      `${sizes[i]} chunks: ${time.toFixed(4)}ms`
    ).join(', '));
    ...
    const avgTimePerChunk = timePerChunk.reduce((sum, t) => sum + t, 0) / timePerChunk.length;
  ...
  it('should maintain performance under concurrent operations', async () => {
    ...
    const addOperations = Array.from({ length: operationsCount }, (_, i) => 
      () => registry.addChunk(createTestChunk(`add-${i}`))
    );
    
    const removeOperations = Array.from({ length: operationsCount / 2 }, (_, i) => 
      () => registry.removeChunk(`add-${i}`)
    );
    
    const getOperations = Array.from({ length: operationsCount }, () => 
      () => registry.size()
    );
    
    const allOperations = [...addOperations, ...removeOperations, ...getOperations];
    ...
    console.log(`${allOperations.length} concurrent operations completed in ${totalTime.toFixed(2)}ms`);
