...
export class ContextPruner {
  ...
  pruneChunks(
    ...
    const mandatoryTokens = mandatoryChunks.reduce((sum, chunk) => sum + chunk.tokens, 0);
    ...
    const selectedCandidates = this.greedySelection(scoredCandidates, remainingBudget);
    ...
    const coherentChunks = this.preserveCoherence(allSelectedChunks, chunks, tokenBudget);

    // Step 7: Sort by original order (timestamp)
    const finalChunks = coherentChunks.sort((a, b) => a.timestamp - b.timestamp);
   ...
   */
  private identifyMandatoryChunks(chunks: ConversationChunk[]): ConversationChunk[] {
   ...
   */
  private scoreAndRankCandidates(chunks: ConversationChunk[]): ConversationChunk[] {
    return chunks
      .map(chunk => ({
        chunk,
  ...
  private preserveCoherence(
    ...
    const currentTokens = coherentChunks.reduce((sum, chunk) => sum + chunk.tokens, 0);
    ...
    if (remainingBudget > 0) {
      ...
      const orphanedResponses = this.findOrphanedResponses(sortedSelected, allChunks);
      
      // Add the user messages that precede orphaned assistant responses
      for (const orphaned of orphanedResponses) {
        const precedingUser = this.findPrecedingUserMessage(orphaned, allChunks);
        if (precedingUser && 
  ...
  private findOrphanedResponses(
    ...
    for (const chunk of sortedSelected) {
      if (chunk.role === 'assistant') {
        ...
        const hasPrecdingUser = sortedSelected.some(c => 
  ...
  private findPrecedingUserMessage(
    ...
    const assistantIndex = sortedChunks.findIndex(c => c.id === assistantChunk.id);
  ...
  private createStats(
    ...
    const prunedTokens = prunedChunks.reduce((sum, chunk) => sum + chunk.tokens, 0);
    const tokensReduced = originalTokens - prunedTokens;
    const reductionPercentage = originalTokens > 0 ? Math.round((tokensReduced / originalTokens) * 100) : 0;
