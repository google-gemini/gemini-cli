...
describe('MemoryIntegration', () => {
    ...
    it('should handle memory loading errors gracefully', async () => {
      ...
      expect(
        result.warnings.some((w) =>
          w.includes('Failed to load memory context'),
        ),
      ).toBe(true);
    });
  });
  ...
  describe('MemoryAwareTool', () => {
    ...
    beforeEach(() => {
      ...
      // Mock memory manager methods
      vi.spyOn(memoryManager, 'getCachedToolResult').mockResolvedValue(
        undefined,
      );
    ...
    it('should execute tool and cache result', async () => {
      ...
      const result = await memoryAwareTool.execute(
        params,
  ...
  describe('End-to-end integration', () => {
    ...
    it('should cache and retrieve tool results', async () => {
      ...
      const result1 = await memoryAware.execute(
        params,
        new AbortController().signal,
      );
      ...
      const result2 = await memoryAware.execute(
        params,
        new AbortController().signal,
      );
