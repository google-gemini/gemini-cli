...
export class ProjectContextManager {
  ...
  async analyzeProject(rootPath: string): Promise<ProjectContext> {
    ...
    const { type, buildSystem, testFramework } = await this.detectProjectType(normalizedRoot);
    ...
    const sourceFiles = await this.collectSourceFiles(normalizedRoot, languages);
  ...
  async detectProjectType(rootPath: string): Promise<{
    ...
    try {
      ...
      if (await this.fileExists(packageJsonPath)) {
        ...
        // Check for Next.js
        if (packageJson.dependencies?.next || await this.fileExists(path.join(rootPath, 'next.config.js'))) {
          return {
            type: 'nextjs',
            buildSystem: 'npm',
        ...
        if (packageJson.dependencies?.typescript || await this.fileExists(path.join(rootPath, 'tsconfig.json'))) {
          return {
            type: 'typescript',
            buildSystem: 'npm',
            testFramework: this.detectTestFramework(packageJson),
  ...
  private detectTestFramework(packageJson: any): string | undefined {
    const testFrameworks = ['vitest', 'jest', 'mocha', 'jasmine', 'cypress', 'playwright'];
   ...
   */
  private async detectPythonTestFramework(rootPath: string): Promise<string | undefined> {
    try {
   ...
   */
  async analyzeDependencies(packageJson: any): Promise<ProjectDependency[]> {
    const dependencies: ProjectDependency[] = [];
   ...
   * Analyze directory structure
   */
  async analyzeDirectoryStructure(rootPath: string, maxDepth: number = 3): Promise<DirectoryNode> {
   ...
   */
  private async buildDirectoryTree(dirPath: string, currentDepth: number, maxDepth: number): Promise<DirectoryNode> {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory() && currentDepth < maxDepth) {
          const childNode = await this.buildDirectoryTree(fullPath, currentDepth + 1, maxDepth);
   ...
   */
  private inferDirectoryPurpose(name: string, children: DirectoryNode[]): string | undefined {
   ...
   */
  async collectSourceFiles(rootPath: string, languages: string[]): Promise<Array<{ path: string; content: string }>> {
   ...
   */
  async detectCodingPatterns(sourceFiles: Array<{ path: string; content: string }>): Promise<CodingPattern[]> {
   ...
   */
  private detectReactPatterns(sourceFiles: Array<{ path: string; content: string }>): CodingPattern[] {
    const patterns: CodingPattern[] = [];
    ...
    if (reactFiles.length > 0) {
      ...
      const componentFiles = reactFiles.filter(f => 
        f.content.includes('React.FC') || f.content.includes(': FC<') || f.content.includes('export const')
      );
      ...
      const propsFiles = reactFiles.filter(f => 
        f.content.includes('interface') && f.content.includes('Props')
      );
   ...
   */
  private detectCustomHookPatterns(sourceFiles: Array<{ path: string; content: string }>): CodingPattern[] {
    const patterns: CodingPattern[] = [];
    const hookFiles = sourceFiles.filter(f => 
      f.content.includes('export const use') || f.content.includes('export function use')
    );
    ...
    if (hookFiles.length > 0) {
      ...
      hookFiles.forEach(f => {
        ...
        if (matches) {
          examples.push(...matches.map(m => m.replace(/export (?:const|function) /, '')));
        }
      });
   ...
   */
  private detectUtilityPatterns(sourceFiles: Array<{ path: string; content: string }>): CodingPattern[] {
    const patterns: CodingPattern[] = [];
    const utilFiles = sourceFiles.filter(f => 
      f.path.includes('util') || f.path.includes('helper') || f.content.includes('export const') || f.content.includes('export function')
    );
   ...
   */
  private detectComponentPatterns(sourceFiles: Array<{ path: string; content: string }>): CodingPattern[] {
    const patterns: CodingPattern[] = [];
    ...
    const indexFiles = sourceFiles.filter(f => 
      f.path.endsWith('index.ts') || f.path.endsWith('index.js')
    );
   ...
   */
  private async detectCodeStylePreferences(rootPath: string, preferences: ProjectPreferences): Promise<void> {
    // Check .prettierrc
    const prettierPath = path.join(rootPath, '.prettierrc');
    if (await this.fileExists(prettierPath)) {
      try {
        const prettierConfig = JSON.parse(await fs.readFile(prettierPath, 'utf-8'));
        if (prettierConfig.tabWidth) preferences.codeStyle.indentSize = prettierConfig.tabWidth;
        if (prettierConfig.useTabs !== undefined) {
          preferences.codeStyle.indentation = prettierConfig.useTabs ? 'tabs' : 'spaces';
        }
        if (prettierConfig.printWidth) preferences.codeStyle.maxLineLength = prettierConfig.printWidth;
      } catch {
        // Ignore errors
      }
    }
   ...
   */
  async detectNamingConventions(sourceFiles: Array<{ path: string; content: string }>): Promise<ProjectPreferences['namingConventions']> {
    const conventions = {
    ...
    const fileNames = sourceFiles.map(f => path.basename(f.path, path.extname(f.path)));
  ...
  async findDocumentationFiles(rootPath: string): Promise<string[]> {
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (relativePath.endsWith('.md') || relativePath.includes('README') || relativePath.includes('LICENSE')) {
   ...
   */
  async updatePatterns(context: ProjectContext, newPatterns: CodingPattern[]): Promise<ProjectContext> {
    const updatedPatterns = [...context.patterns];
    
    for (const newPattern of newPatterns) {
      const existingIndex = updatedPatterns.findIndex(p => p.name === newPattern.name);
      if (existingIndex >= 0) {
   ...
   */
  async updateDependencies(context: ProjectContext, newDependencies: ProjectDependency[]): Promise<ProjectContext> {
    const updatedDependencies = [...context.dependencies];
    
    for (const newDep of newDependencies) {
      const existingIndex = updatedDependencies.findIndex(d => d.name === newDep.name && d.type === newDep.type);
      if (existingIndex >= 0) {
