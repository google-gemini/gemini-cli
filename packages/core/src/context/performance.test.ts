...
describe('Context Optimization Performance and Validation', () => {
  ...
  const createMockChunk = (
    ...
    tokens: tokens || Math.max(10, (content || `Content for chunk ${id}`).length / 4),
  ...
  const generateLargeConversationDataset = (size: number): ConversationChunk[] => {
    const chunks: ConversationChunk[] = [];
    const topics = [
      'machine learning', 'web development', 'data science', 'artificial intelligence',
      'software engineering', 'cloud computing', 'cybersecurity', 'mobile development',
      'database design', 'user experience', 'project management', 'system architecture'
    ];
  ...
  describe('Token Reduction Effectiveness', () => {
    it('should achieve significant token reduction under budget pressure', async () => {
      ...
      const result = await contextManager.optimizeContext(query, aggressiveBudget);
    ...
    it('should maintain efficiency across different optimization strategies', async () => {
      ...
      const standardResult = await contextManager.optimizeContext(query, budget);
      const standardStats = contextManager.getOptimizationStats();
      
      // Test aggressive strategy
      contextManager.updateConfig({ ...config, aggressivePruning: true });
      const aggressiveResult = await contextManager.optimizeContext(query, budget);
  ...
  describe('Relevance Preservation Accuracy', () => {
    it('should prioritize highly relevant chunks', async () => {
      ...
      const chunks: (ConversationChunk & { groundTruthRelevance?: number })[] = [
        createMockChunk('high-rel-1', 'user', 'machine learning neural networks deep learning', 100, Date.now() - 5000, 0.9),
        createMockChunk('high-rel-2', 'assistant', 'artificial intelligence and machine learning algorithms', 120, Date.now() - 4000, 0.85),
        createMockChunk('med-rel-1', 'user', 'programming and software development', 80, Date.now() - 3000, 0.6),
        createMockChunk('low-rel-1', 'user', 'weather forecast for tomorrow', 60, Date.now() - 2000, 0.2),
        createMockChunk('low-rel-2', 'assistant', 'cooking recipes and food preparation', 90, Date.now() - 1000, 0.1),
      ];
      
      contextManager.addChunks(chunks);
      
      const query: RelevanceQuery = { text: 'machine learning artificial intelligence' };
      ...
      expect(highRelevanceIncluded.length).toBeGreaterThan(lowRelevanceIncluded.length);
    });
    ...
    it('should calculate relevance preservation metrics accurately', async () => {
      ...
      chunks.forEach((chunk, index) => {
        const chunkWithTruth = chunk as ConversationChunk & { groundTruthRelevance?: number };
        // Simulate realistic relevance distribution
        if (chunk.content.includes('machine learning') || chunk.content.includes('artificial intelligence')) {
          chunkWithTruth.groundTruthRelevance = 0.8 + Math.random() * 0.2;
        } else if (chunk.content.includes('data science') || chunk.content.includes('web development')) {
      ...
      const selectedWithTruth = result.chunks.map(chunk => {
        const original = chunks.find(c => c.id === chunk.id) as ConversationChunk & { groundTruthRelevance?: number };
        return original?.groundTruthRelevance || 0;
      });
      
      const relevantSelected = selectedWithTruth.filter(score => score > 0.6).length;
    ...
    it('should maintain relevance quality across different query types', async () => {
      ...
      const queryTypes = [
        { text: 'machine learning algorithms', expectedTopic: 'machine learning' },
        { text: 'web development frameworks', expectedTopic: 'web development' },
        { text: 'database design patterns', expectedTopic: 'database' },
        { text: 'user experience principles', expectedTopic: 'user experience' },
      ];
      ...
      for (const queryType of queryTypes) {
        ...
        const topicallyRelevant = result.chunks.filter(chunk => 
          chunk.content.toLowerCase().includes(queryType.expectedTopic.toLowerCase())
        ).length;
  ...
  describe('Throughput and Scalability', () => {
    ...
    it('should scale to handle very large conversations', async () => {
      ...
      const throughputRatio = performanceData[2].throughput / performanceData[0].throughput;
  ...
  describe('Quality Validation Metrics', () => {
    it('should maintain conversation coherence under optimization pressure', async () => {
      ...
      const query: RelevanceQuery = { text: 'question and response about topics' };
      ...
      const sortedResult = result.chunks.sort((a, b) => a.timestamp - b.timestamp);
      ...
      for (let i = 0; i < sortedResult.length; i++) {
        ...
        if (chunk.role === 'assistant') {
          ...
          const hasUserBefore = precedingChunks.some(c => 
            c.role === 'user' && c.timestamp < chunk.timestamp
          );
    ...
    it('should preserve critical information markers', async () => {
      const systemChunk = createMockChunk('system', 'assistant', 'System prompt information', 100, Date.now() - 10000);
      systemChunk.metadata.tags = ['system-prompt'];
      
      const importantChunk = createMockChunk('important', 'user', 'Critical user information', 100, Date.now() - 9000);
      importantChunk.metadata.pinned = true;
      
      const toolChunk = createMockChunk('tool', 'tool', 'Tool definition', 150, Date.now() - 8000);
      ...
      const hasPinnedChunk = result.chunks.some(chunk => 
    ...
    it('should provide comprehensive performance reporting', async () => {
      ...
      expect(cumulativeStats.averageReductionPercentage).toBeGreaterThanOrEqual(0);
    });
  });
