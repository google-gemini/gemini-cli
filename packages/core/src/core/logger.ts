...
export class Logger {
  ...
  async initialize(): Promise<void> {
    ...
    this.llmcliDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.llmcliDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.llmcliDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.llmcliDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.llmcliDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.llmcliDir) {
      debugLogger.error(
