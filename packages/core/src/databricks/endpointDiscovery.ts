...
export async function discoverDatabricksEndpoints(options: DiscoveryOptions = {}): Promise<string[]> {
  // Check credentials
    ...
    do {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          throw new Error('Permission denied: User lacks access to serving endpoints');
