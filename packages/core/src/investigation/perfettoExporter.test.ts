...
describe('PerfettoExporter', () => {
  ...
  describe('constructor', () => {
    it('should add process and thread metadata events', () => {
      ...
      const processNameEvent = events.find(
        (e) => e.ph === 'M' && e.name === 'process_name',
      );
      const threadNameEvent = events.find(
  ...
  describe('exportLeakReport()', () => {
    const mockReport: LeakReport = {
      ...
      snapshotSizes: [1_000_000, 1_500_000, 2_000_000] as [
        number,
        number,
        number,
      ],
    ...
    it('should include memory counter events', () => {
      const trace = exporter.exportLeakReport(mockReport);
      const counterEvents = trace.traceEvents.filter(
        (e) => e.ph === 'C' && e.name === 'Heap Size',
      );
    ...
    it('should include leak annotations', () => {
      const trace = exporter.exportLeakReport(mockReport);
      const leakEvents = trace.traceEvents.filter((e) =>
        e.name?.startsWith('LEAK:'),
      );
    ...
    it('should include analysis phase event', () => {
      const trace = exporter.exportLeakReport(mockReport);
      const analysisEvent = trace.traceEvents.find(
        (e) => e.name === 'Leak Analysis',
      );
