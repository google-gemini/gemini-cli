...
export class OAuthUtils {
  ...
  /**
   * Validates that two URLs have the same origin.
   * Implements RFC 9728 Section 3.2 origin validation to prevent SSRF attacks.
   *
   * @param url1 First URL to compare
   * @param url2 Second URL to compare
   * @param errorContext Context for error message (e.g., "metadata URL", "resource URL")
   * @throws ResourceMismatchError if origins don't match or URLs are invalid
   */
  private static validateSameOrigin(
    url1: string,
    url2: string,
    errorContext: string,
  ): void {
    try {
      ...
      if (parsedUrl1.origin !== parsedUrl2.origin) {
        throw new ResourceMismatchError(
          `${errorContext} origin ${parsedUrl1.origin} does not match expected origin ${parsedUrl2.origin}`,
        );
      }
    } catch (error) {
      if (error instanceof ResourceMismatchError) {
      ...
      // If URL parsing fails, reject to fail closed (security-first approach)
      debugLogger.debug(
        `Failed to parse URLs for origin validation (${errorContext}): ${getErrorMessage(error)}`,
      );
      throw new ResourceMismatchError(`Invalid URL in ${errorContext}`);
    }
  }
  ...
  static async discoverOAuthFromWWWAuthenticate(
    ...
    // RFC 9728 Section 3.2: Verify that the metadata URL origin matches the server origin
    // to prevent SSRF attacks where a malicious server could cause us to fetch arbitrary URLs
    if (mcpServerUrl) {
      try {
    ...
    if (resourceMetadata && mcpServerUrl) {
      ...
      // Verify that the resource URL origin also matches before processing
      // This prevents information disclosure if an attacker-controlled internal service
      // returns valid JSON with a different origin in the resource field
      this.validateSameOrigin(
        resourceMetadata.resource,
        mcpServerUrl,
        'Protected resource',
  ...
  /**
   * Normalizes a URL for resource comparison by:
   * 1. Stripping query parameters and fragments (to match buildResourceParameter behavior)
   * 2. Removing trailing slash for root paths only (to handle URL.pathname always being '/')
   *
   * This ensures consistent comparison between server-provided resource URLs and
   * expected resource URLs generated by buildResourceParameter.
   *
  ...
  static normalizeResourceUrlForComparison(url: string): string {
    ...
    // For root paths, remove trailing slash (URL() always adds it)
    if (urlObj.pathname === '/') {
