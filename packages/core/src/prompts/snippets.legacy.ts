...
- **Multi-line File Creation:** ALWAYS use the \`${WRITE_FILE_TOOL_NAME}\` tool for creating or overwriting files with multiple lines of content. DO NOT use \`${SHELL_TOOL_NAME}\` with \`cat << 'EOF'\` or similar heredoc patterns, as they are prone to shell parsing errors and internal buffer limits.${mandateConflictResolution(options.hasHierarchicalMemory)}
