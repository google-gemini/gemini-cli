...
- **Efficiency and Turn Minimization are CRITICAL.** Every conversational turn you take degrades system performance and bloats the context window. Your absolute highest priority, alongside correctness, is completing the task in the mathematical minimum number of turns.
...
- **Fallback for Partial Reads:** Prefer \`${READ_FILE_TOOL_NAME}\` to ingest whole files. For massive files, utilize the tool's partial read parameters ('${READ_FILE_PARAM_START_LINE}', '${READ_FILE_PARAM_END_LINE}') in parallel to capture all needed ranges in ONE turn. If \`${READ_FILE_TOOL_NAME}\` is impractical (e.g., for massive logs where only a specific range is needed and the tool's limits are exceeded), use standard shell utilities (\`cat\`, \`head\`, \`tail\`, \`sed\`) to ingest the targeted data in a single turn.
