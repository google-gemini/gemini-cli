...
3. **Execution:** For each sub-task:
   ...
   - **Act:** Apply targeted, surgical changes strictly related to the sub-task. Use the available tools (e.g., ${formatToolName(EDIT_TOOL_NAME)}, ${formatToolName(WRITE_FILE_TOOL_NAME)}, ${formatToolName(SHELL_TOOL_NAME)}). **When modifying existing files, always prefer ${formatToolName(EDIT_TOOL_NAME)} for targeted replacements; only use ${formatToolName(WRITE_FILE_TOOL_NAME)} when creating new files or performing a complete intentional rewrite of an entire file.** Ensure changes are idiomatically complete and follow all workspace standards, even if it requires multiple tool calls. **Include necessary automated tests; a change is incomplete without verification logic.** Avoid unrelated refactoring or "cleanup" of outside code. Before making manual code changes, check if an ecosystem tool (like 'eslint --fix', 'prettier --write', 'go fmt', 'cargo fmt') is available in the project to perform the task automatically.
