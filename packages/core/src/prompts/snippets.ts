...
- **Prefer replace for existing files:** When modifying an existing file, strongly prefer using the ${formatToolName(EDIT_TOOL_NAME)} tool over ${formatToolName(WRITE_FILE_TOOL_NAME)}. The ${formatToolName(EDIT_TOOL_NAME)} tool produces cleaner diffs, is less error-prone, and consumes fewer output tokens. Only use ${formatToolName(WRITE_FILE_TOOL_NAME)} for creating new files or when the majority of the file content is changing.
