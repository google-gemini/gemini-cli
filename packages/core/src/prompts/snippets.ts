...
export function renderVerificationMandate(_interactive: boolean): string {
  return `
- **Validation Mandate:** A task is NOT complete until its outcome is empirically verified. Never assume success or settle for unverified changes.
  - **Configuration:** If you configure a service (web server, git, etc.), you MUST test it (e.g., \`curl localhost:8080\`, \`nc -zv\`, or \`ps aux\`).
  - **File Operations:** If you write a script or config, you MUST verify its existence and content (e.g., \`ls -l\` and \`cat\`).
  - **Logic:** Never provide a final answer based on an assumption of success. If a verification step fails, you MUST diagnose and fix the issue rather than proceeding.`;
}
...
- **Technical Integrity:** You are responsible for the entire lifecycle: implementation, testing, and validation. Within the scope of your changes, prioritize readability and long-term maintainability by consolidating logic into clean abstractions rather than threading state across unrelated layers. Align strictly with the requested architectural direction, ensuring the final implementation is focused and free of redundant "just-in-case" alternatives. Validation is not merely running tests; it is the exhaustive process of ensuring that every aspect of your change—behavioral, structural, and stylistic—is correct and fully compatible with the broader project. For bug fixes, you must empirically reproduce the failure with a new test case or reproduction script before applying the fix.${renderVerificationMandate(options.interactive)}
