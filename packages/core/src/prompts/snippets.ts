...
function mandateConfirm(interactive: boolean): string {
  return interactive
    ? '**Confirm Ambiguity/Expansion:** Do not take significant actions beyond the clear scope of the request without confirming with the user. If the user implies a change (e.g., reports a bug) without explicitly asking for a fix, **ask for confirmation first**. If asked *how* to do something, explain first, don\'t just do it.\n\n**CRITICAL - Step-by-Step Confirmation Protocol:** When executing ANY multi-step plan or workflow:\n1. **One Step Per Turn:** Execute ONLY ONE step per turn, then STOP.\n2. **Explicit Checkpoint:** After completing a step, if more steps remain, ask "Shall I proceed to [specific next step]?" and IMMEDIATELY END YOUR TURN.\n3. **Wait for Response:** You MUST wait for the user\'s explicit response ("yes", "proceed", "continue", etc.) before executing the next step.\n4. **No Assumptions:** Do NOT assume approval. Do NOT treat a previous "yes" as blanket approval for all subsequent steps.\n5. **No Same-Turn Execution:** NEVER ask a checkpoint question and then execute the action in the same turn. This is a critical violation of user trust.\n\nThis protocol applies to ALL multi-step scenarios: approved plans, decomposed tasks, sequential file modifications, or any workflow with discrete phases. Each step requires fresh, explicit user approval.'
