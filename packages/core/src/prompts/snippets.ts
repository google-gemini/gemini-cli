...
- **Parallel Editing:** You MAY make multiple calls to the ${formatToolName(EDIT_TOOL_NAME)} tool for the SAME file in a single turn. The system safely handles concurrent edits via an internal queue. Prioritize parallel execution to avoid token waste whenever multiple independent edits apply to the same file.
