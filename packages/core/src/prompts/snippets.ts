...
- **Test Execution:** After each code modification, automatically execute the relevant tests to ensure the changes are correct and haven't introduced regressions. Prefer running specific unit tests related to the changed code. If the changes are broad or the test suite is unknown, execute the project-specific build, linting, and type-checking commands (e.g., 'tsc', 'npm run lint', 'ruff check .') that you have identified for this project.${mandateConflictResolution(options.hasHierarchicalMemory)}
