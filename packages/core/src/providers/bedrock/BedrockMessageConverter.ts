...
export class BedrockMessageConverter {
  ...
  convertToBedrockMessages(contents: Content[]): BedrockMessageParam[] {
    ...
    for (const content of contents) {
      if (content.role === 'user' || content.role === 'model') {
        const bedrockContent = this.convertPartsToBedrockContent(
          content.parts || [],
        );
        messages.push({
   ...
   */
  private convertPartsToBedrockContent(
    parts: Part[],
  ): string | BedrockContentBlockParam[] {
    ...
    for (const part of parts) {
      if ('text' in part && part.text) {
        ...
        blocks.push({
          ...
          source: {
            type: 'base64',
            media_type: (part.inlineData.mimeType || 'image/jpeg') as
              | 'image/jpeg'
              | 'image/png'
              | 'image/gif'
              | 'image/webp',
        ...
        if (!toolUseId && part.functionResponse.name) {
          console.warn(
            `No tool use ID found for function response: ${part.functionResponse.name}`,
          );
        }
