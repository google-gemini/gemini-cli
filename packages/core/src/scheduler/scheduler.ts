    ...
    let isSandboxError = false;
      ...
      if (isSandboxError) {
        try {
          ...
          this.state.updateStatus(callId, CoreToolCallStatus.AwaitingApproval, { confirmationDetails, correlationId });
  
          const validatingCall = { ...activeCall, status: CoreToolCallStatus.Validating } as ValidatingToolCall;
          ...
          if (confResult.outcome === ToolConfirmationOutcome.Cancel) {
            ...
            type LegacyHack = ToolCallResponseInfo & { llmContent?: string; returnDisplay?: string; };
          ...
          this.state.updateStatus(callId, CoreToolCallStatus.Executing, { liveOutput: undefined });
  
          // Call _execute synchronously and properly return its promise to loop internally!
          return await this._execute({ ...activeCall, status: CoreToolCallStatus.Scheduled } as ScheduledToolCall, signal);
        } catch (_e) {
