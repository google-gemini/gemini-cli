...
import type {
  ExtractionState,
  ExtractionRun,
} from './defaultMemoryProvider.js';
import { coreEvents } from '../utils/events.js';
import { Storage } from '../config/storage.js';
import { debugLogger } from '../utils/debugLogger.js';

// Mock external modules used by DefaultMemoryProvider
...
describe('memoryService', () => {
  ...
  describe('tryAcquireLock', () => {
    it('successfully acquires lock when none exists', async () => {
      const { tryAcquireLock } = await import('./defaultMemoryProvider.js');
  ...
  describe('isLockStale', () => {
    it('returns true when PID is dead', async () => {
      const { isLockStale } = await import('./defaultMemoryProvider.js');
  ...
  describe('releaseLock', () => {
    it('deletes the lock file', async () => {
      const { releaseLock } = await import('./defaultMemoryProvider.js');
  ...
  describe('DefaultMemoryProvider', () => {
    it('skips when lock is held by another instance', async () => {
      const { DefaultMemoryProvider } = await import(
        './defaultMemoryProvider.js'
      );
      ...
      provider.onSessionStart(mockConfig, 'session-id');

      await vi.waitFor(() => {
        expect(debugLogger.log).toHaveBeenCalledWith(
          '[MemoryService] Skipped: lock held by another instance',
        );
      });
    ...
    it('starts skill extraction in the background on session start', async () => {
      const { DefaultMemoryProvider } = await import(
        './defaultMemoryProvider.js'
      );
      const { LocalAgentExecutor } = await import(
      ...
      const memoryDir = path.join(tmpDir, 'memory-background');
      const skillsDir = path.join(tmpDir, 'skills-background');
      const projectTempDir = path.join(tmpDir, 'temp-background');
      ...
      await fs.writeFile(
        path.join(chatsDir, 'session-2025-01-01T00-00-background.json'),
      ...
      await vi.waitFor(() => {
        expect(ExecutionLifecycleService.completeExecution).toHaveBeenCalled();
      });
    });
  ...
  describe('getProcessedSessionIds', () => {
    it('returns empty set for empty state', async () => {
      const { getProcessedSessionIds } = await import(
        './defaultMemoryProvider.js'
      );
  ...
  describe('buildSessionIndex', () => {
    ...
    it('returns empty index and no new IDs when chats dir is empty', async () => {
      const { buildSessionIndex } = await import('./defaultMemoryProvider.js');
  ...
  describe('validatePatches', () => {
    ...
    it('returns empty array when no patch files exist', async () => {
      const { validatePatches } = await import('./defaultMemoryProvider.js');
  ...
  describe('DefaultMemoryProvider feedback for patch-only runs', () => {
