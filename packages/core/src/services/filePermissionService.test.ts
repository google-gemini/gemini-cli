...
describe('FilePermissionService', () => {
  ...
  describe('Basic Allow/Deny', () => {
    it('should deny by default if no rules match', () => {
      ...
      expect(
        service.canPerformOperation(path.join(targetDir, 'file.txt'), 'read'),
      ).toBe(false);
    });
  ...
  describe('Pattern Matching', () => {
    it('should allow with wildcard pattern', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'another.txt'),
          'read',
        ),
      ).toBe(true);
      expect(
        service.canPerformOperation(path.join(targetDir, 'file.md'), 'read'),
      ).toBe(false);
    });
    ...
    it('should allow with directory wildcard pattern', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'src/app/component.ts'),
          'write',
        ),
      ).toBe(true);
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'src/legacy/main.js'),
          'write',
        ),
      ).toBe(false);
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'test/app/component.test.ts'),
          'write',
        ),
      ).toBe(false);
    });
    ...
    it('should handle multiple patterns in a single rule', () => {
      ...
      expect(
        service.canPerformOperation(path.join(targetDir, 'README.md'), 'read'),
      ).toBe(true);
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'docs/guide.txt'),
          'read',
        ),
      ).toBe(true);
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'src/code.ts'),
          'read',
        ),
      ).toBe(false);
    });
  ...
  describe('Operation Matching', () => {
    ...
    it('should allow if rule includes multiple operations', () => {
      ...
      expect(
        service.canPerformOperation(path.join(targetDir, 'user.data'), 'read'),
      ).toBe(true);
      expect(
        service.canPerformOperation(path.join(targetDir, 'user.data'), 'write'),
      ).toBe(true);
    });
  });
  ...
  describe('Path Normalization and Security', () => {
    it('should deny paths outside targetDir', () => {
      setupServiceWithRules([
        // Allow all reads within targetDir for this test
    ...
    it('should correctly handle relative paths for patterns (relative to targetDir)', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'src/file.ts'),
          'read',
        ),
      ).toBe(true);
      expect(
        service.canPerformOperation(path.join(targetDir, 'file.ts'), 'read'),
      ).toBe(false);
    });
    ...
    it('should match dotfiles with patterns explicitly including them or if dot:true is default', () => {
      ...
      expect(
        service.canPerformOperation(path.join(targetDir, '.configrc'), 'read'),
      ).toBe(true);
      expect(
        service.canPerformOperation(path.join(targetDir, 'src/.env'), 'read'),
      ).toBe(true);
  ...
  describe('Complex Scenarios', () => {
    const rules: FilePermissionRule[] = [
      {
        description: 'Allow read for all .md files',
        patterns: ['**/*.md'],
        operations: ['read'],
        effect: 'allow',
      },
      {
        description: 'Deny write for anything in secure/',
        patterns: ['secure/**/*'],
        operations: ['write'],
        effect: 'deny',
      },
      {
        description: 'Allow write for specific secure config',
        patterns: ['secure/config.json'],
        operations: ['write'],
        effect: 'allow',
      },
      {
        description: 'Deny read for all .key files',
        patterns: ['**/*.key'],
        operations: ['read'],
        effect: 'deny',
      },
      {
        description: 'Allow all operations for temp files',
        patterns: ['temp/*'],
        operations: ['read', 'write'],
        effect: 'allow',
      },
      {
        description: 'Deny everything else in temp/critical',
        patterns: ['temp/critical/**'],
        operations: ['read', 'write'],
        effect: 'deny',
      },
    ];
    ...
    it('Scenario 1: Read markdown - allowed', () => {
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'docs/guide.md'),
          'read',
        ),
      ).toBe(true);
    });
    ...
    it('Scenario 2: Write to secure file - denied', () => {
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'secure/data.txt'),
          'write',
        ),
      ).toBe(false);
    });
    ...
    it('Scenario 3: Write to specific secure config - allowed (due to order)', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'secure/config.json'),
          'write',
        ),
      ).toBe(true);
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'secure/other.txt'),
          'write',
        ),
      ).toBe(false); // Still denied by the broader rule
    });

    it('Scenario 4: Read .key file - denied', () => {
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'secrets/api.key'),
          'read',
        ),
      ).toBe(false);
    });
    ...
    it('Scenario 5: Write to temp file - allowed', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'temp/log.txt'),
          'write',
        ),
      ).toBe(true);
    });
    ...
    it('Scenario 6: Read from temp/critical/file.txt - denied', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'temp/critical/important.dat'),
          'read',
        ),
      ).toBe(false);
    });
    ...
    it('Scenario 7: Read from temp/allowed.txt - allowed', () => {
      ...
      expect(
        service.canPerformOperation(
          path.join(targetDir, 'temp/allowed.txt'),
          'read',
        ),
      ).toBe(true);
    });
