...
describe('memoryService', () => {
  ...
  describe('getUnprocessedSessions', () => {
    ...
    it('filters out subagent sessions', async () => {
      ...
      await fs.writeFile(
        path.join(chatsDir, 'session-2025-01-01T00-00-sub00001.json'),
      ...
      const result = await getUnprocessedSessions(chatsDir, state);
    ...
    it('filters out already processed sessions', async () => {
      ...
      const conversation = createConversation({
        sessionId: 'already-done',
      });
      await fs.writeFile(
        path.join(chatsDir, 'session-2025-01-01T00-00-done0001.json'),
        JSON.stringify(conversation),
      );
    ...
    it('filters out sessions with fewer than 3 user messages', async () => {
      ...
      await fs.writeFile(
        path.join(chatsDir, 'session-2025-01-01T00-00-short001.json'),
        JSON.stringify(conversation),
      );
    ...
    it('returns sessions sorted most recent first, up to cap', async () => {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        await fs.writeFile(
          path.join(
            chatsDir,
            `session-2025-01-0${i + 1}T00-00-idx${paddedIndex}.json`,
          ),
          JSON.stringify(conversation),
        );
      }
      ...
      expect(result[0].sessionId).toBe('session-6');
      expect(result[4].sessionId).toBe('session-2');
    });

    it('returns empty array if chats dir does not exist', async () => {
      const { getUnprocessedSessions } = await import('./memoryService.js');
      ...
      const result = await getUnprocessedSessions(nonexistentDir, state);
