...
export interface ModelDefinition {
  ...
  isPreview?: boolean;
  // Whether the model is an auto model.
  isAuto?: boolean;
  ...
  features?: {
    ...
    // Whether the model should be treated as a modern model. This includes
    // Gemini 3 models and any custom models.
    modern?: boolean;
  };
}
...
export class ModelConfigService {
  ...
  getModelDefinition(modelId: string): ModelDefinition | undefined {
    ...
    if (!modelId.startsWith('gemini-')) {
      return {
        ...
        family: 'custom',
        features: {
          modern: true, // Custom models are treated as modern in legacy code
        },
      };
    }
  ...
  private matches(
    ...
    if (
      condition.useGemini3_1 !== undefined &&
      condition.useGemini3_1 !== context.useGemini3_1
    ) {
      return false;
    }
    if (
    ...
    if (
      condition.requestedModels !== undefined &&
      (!context.requestedModel ||
        !condition.requestedModels.includes(context.requestedModel))
    ) {
      return false;
    }
    return true;
