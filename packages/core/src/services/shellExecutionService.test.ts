...
describe('ShellExecutionService child_process fallback', () => {
  ...
  beforeEach(() => {
    ...
    // No stdout/stderr when stdio is 'inherit'
  ...
  const simulateExecution = async (
    ...
    const handle = await ShellExecutionService.execute(
      ...
      onOutputEventMock,
      abortController.signal,
      true, // Try to use PTY, but will fallback
      shellExecutionConfig,
    );
  ...
  describe('Interactive Execution', () => {
    it('should execute a command with inherited stdio', async () => {
      ...
      expect(result.aborted).toBe(false);
      expect(result.output).toBe(
        '[Output not captured for interactive command]',
      );
      expect(handle.pid).toBe(undefined);

      expect(onOutputEventMock).toHaveBeenCalledWith({
        type: 'data',
        chunk: '[Output not captured for interactive command]',
      });
    });
  ...
  describe('Platform-Specific Behavior', () => {
    ...
    it('should use bash and non-detached process on Linux', async () => {
      mockPlatform.mockReturnValue('linux');
