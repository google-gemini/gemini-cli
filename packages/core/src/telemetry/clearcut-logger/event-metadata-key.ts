...
export enum EventMetadataKey {
  ...
  GEMINI_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_SYSTEM_INSTRUCTIONS = 159,

  // Logs the token count for tool definitions.
  GEMINI_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_TOOL_DEFINITIONS = 160,

  // Logs the token count for conversation history.
  GEMINI_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_HISTORY = 161,

  // Logs the token count for tool calls (JSON map of tool name to tokens).
  GEMINI_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_TOOL_CALLS = 162,

  // Logs the token count from MCP servers (tool definitions + tool inputs/outputs).
  GEMINI_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_MCP_SERVERS = 163,
