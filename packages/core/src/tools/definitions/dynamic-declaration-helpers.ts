...
export function getShellToolDescription(
  ...
  if (os.platform() === 'win32') {
    ...
    const backgroundInstructions = enableInteractiveShell
      ? `To run a command in the background, set the \`${SHELL_PARAM_IS_BACKGROUND}\` parameter to true. For background execution, use is_background: true instead of &.`
      : 'Command can start background processes using `&`.';
    return `Executes a shell command via bash -c. ${backgroundInstructions} Command is executed as a subprocess that leads its own process group. Command process group can be terminated as \`kill -- -PGID\` or signaled as \`kill -s SIGNAL -- -PGID\`.${efficiencyGuidelines}${returnedInfo}`;
  }
}
