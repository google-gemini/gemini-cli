...
export const GEMINI_3_SET: CoreToolSet = {
  ...
  write_file: {
    ...
    description: `Writes the complete content to a file, automatically creating missing parent directories. Overwrites existing files. PREFERRED over shell-based alternatives (heredocs, cat, echo, tee, or scripts) for creating or overwriting files. The user has the ability to modify 'content' before it is saved. Best for new or small files. This tool CANNOT perform targeted edits like inserting or modifying specific lines; ALWAYS use '${EDIT_TOOL_NAME}' for editing existing files.`,
    parametersJsonSchema: {
      type: 'object',
      properties: {
        [PARAM_FILE_PATH]: {
