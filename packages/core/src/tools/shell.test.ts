...
describe('ShellTool', () => {
  ...
  describe('allowCommands pattern matching', () => {
    ...
    describe('matchesAllowPattern', () => {
      ...
      describe('regex patterns', () => {
        ...
        it('should match complex regex patterns', () => {
          expect(shellTool.matchesAllowPattern('npm test', '/^npm\\s+(test|install)$/')).toBe(true);
          expect(shellTool.matchesAllowPattern('npm install', '/^npm\\s+(test|install)$/')).toBe(true);
          expect(shellTool.matchesAllowPattern('npm run', '/^npm\\s+(test|install)$/')).toBe(false);
        });

        it('should handle invalid regex gracefully', () => {
          const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
          expect(shellTool.matchesAllowPattern('ls', '/[/')).toBe(false);
          expect(warnSpy).toHaveBeenCalledWith('Invalid regex pattern: /[/');
          warnSpy.mockRestore();
        });

        it('should not treat non-regex patterns as regex', () => {
          expect(shellTool.matchesAllowPattern('/usr/bin/ls', '/usr/bin/ls')).toBe(true);
          expect(shellTool.matchesAllowPattern('test/path', 'test/path')).toBe(true);
        });
      });
      ...
      describe('edge cases', () => {
        ...
        it('should handle whitespace', () => {
          expect(shellTool.matchesAllowPattern('git status', 'git status')).toBe(true);
          expect(shellTool.matchesAllowPattern('git  status', 'git status')).toBe(false);
        });

        it('should handle special characters', () => {
          expect(shellTool.matchesAllowPattern('git-flow', 'git-flow')).toBe(true);
          expect(shellTool.matchesAllowPattern('npm@latest', 'npm@latest')).toBe(true);
          expect(shellTool.matchesAllowPattern('test_command', 'test_command')).toBe(true);
        });
      });
    });
    ...
    describe('getCommandRoot', () => {
      ...
      it('should extract command from paths', () => {
        ...
        expect(shellTool.getCommandRoot('C:\\Windows\\System32\\cmd.exe')).toBe('cmd.exe');
      });
    ...
    describe('shouldConfirmExecute with allowCommands', () => {
      ...
      it('should skip confirmation for allowed regex patterns', async () => {
        vi.mocked(config.getAllowCommands).mockReturnValue(['/^make(\\s+.*)?$/']);
  ...
  describe('denyCommands', () => {
    ...
    it('should require confirmation for commands matching deny patterns', async () => {
      vi.mocked(config.getDenyCommands).mockReturnValue(['rm', 'del*', '/^sudo/']);
      vi.mocked(config.getAllowCommands).mockReturnValue(undefined);
    ...
    it('should check denyCommands before allowCommands', async () => {
      ...
      vi.mocked(config.getDenyCommands).mockReturnValue(['rm -rf', 'git push --force']);
      
      // rm is allowed, but rm -rf is denied (only command root is checked)
      const result1 = await shellTool.shouldConfirmExecute(
        { command: 'rm file.txt' },
        new AbortController().signal
      );
      ...
      // git is allowed with glob, but git push --force is denied
      const result2 = await shellTool.shouldConfirmExecute(
        { command: 'git status' },
        new AbortController().signal
      );
    ...
    it('should add denied commands to whitelist if user confirms with always', async () => {
      ...
      if (confirmResult && 'onConfirm' in confirmResult && confirmResult.onConfirm) {
    ...
    it('should support complex deny patterns', async () => {
      vi.mocked(config.getDenyCommands).mockReturnValue([
        '/^rm\\s+-rf/',           // regex: rm -rf commands
        'sudo*',                  // glob: any sudo command
        'chmod 777',              // exact: specific chmod
      ...
      const testCases = [
        { command: 'rm -rf /', shouldDeny: false }, // Only checks command root 'rm'
        { command: 'sudo apt update', shouldDeny: true },
        { command: 'chmod 777 /etc/passwd', shouldDeny: false }, // Only checks 'chmod', not the full command
      ...
      for (const testCase of testCases) {
        ...
        if (testCase.shouldDeny) {
          ...
          if (result && typeof result !== 'boolean') {
            ...
            expect((result as { title: string }).title).not.toBe('Confirm Denied Command');
          }
        }
      }
    });
    ...
    it('should work with allowCommands and denyCommands together', async () => {
      vi.mocked(config.getAllowCommands).mockReturnValue(['git*', 'npm*', 'ls']);
      vi.mocked(config.getDenyCommands).mockReturnValue(['git push --force', 'npm publish', 'sudo*']);
      ...
      // Denied despite matching allow pattern (only checks command root)
      result = await shellTool.shouldConfirmExecute(
        { command: 'git push --force' },
        new AbortController().signal
      );
      expect(result).toBe(false); // git is allowed, only 'git' is checked
