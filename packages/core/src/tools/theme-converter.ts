...
export function convertVSCodeThemeToCustomTheme(vscodeTheme: VSCodeTheme): CustomTheme {
  ...
  const diffModifiedColor = colors['diffEditor.modifiedTextBackground'] ||
  ...
  const diffModified = diffModifiedColor || generateDiffColor(accentBlue, 'modified');

  // Generate gradient colors based on theme's accent colors
  const gradientColors = generateGradientColors(accentBlue, accentPurple, accentCyan);
 ...
 */
export function generateDiffColor(baseColor: string, _type: 'added' | 'removed' | 'modified'): string {
 ...
 */
export function generateGradientColors(accentBlue: string, accentPurple: string, accentCyan: string): string[] {
