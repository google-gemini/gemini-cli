...
async function uploadFile() {
  ...
  readline.question(chalk.blue('Enter the file to upload (e.g., /data/data/com.termux/files/home/file.txt): '), async (filePath) => {
    ...
    readline.question(chalk.blue('Enter the server URL (e.g., https://example.com/upload): '), async (serverUrl) => {
      console.log(chalk.cyan(`// Channeling '${filePath}' to the server...`));
      const form = new FormData();
      form.append('file', fs.createReadStream(filePath), path.basename(filePath));
      try {
        const response = await axios.post(serverUrl, form, { headers: form.getHeaders() });
        console.log(chalk.green(`Success! '${filePath}' has ascended to the server: ${response.status}`));
