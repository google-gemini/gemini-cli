 ...
 */
export async function extractVsixContent(vsixBuffer: Buffer): Promise<VsixContent> {
  ...
  try {
    ...
    throw new Error(`Failed to extract VSIX content: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
 ...
 */
function readCentralDirectoryEntry(buffer: Buffer, offset: number): ZipEntry | null {
