...
async function _processCrawlResults(
  ...
  // fdir always includes the crawled directory as the first result, but we
  ...
  if (options.crawlDirectory === options.cwd) {
    ...
    for (const [i, p] of rawResults.entries()) {
      ...
      // If the original path was a directory (ended with '/'), ensure the
      // relative path also ends with '/'
