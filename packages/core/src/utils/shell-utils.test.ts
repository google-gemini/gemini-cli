...
describe('getCommandPrefix', () => {
  const stemmableCommands = ['git', 'npm', 'npx', 'gh', 'gh run'];

  it('should return only the root command for a simple command', () => {
    const command = 'ls -l';
    const prefix = getCommandPrefix(command, stemmableCommands);
  ...
  it('should return the most specific prefix', () => {
    ...
    const customStemmable = ['git', 'git checkout'];
    const prefix = getCommandPrefix(command, customStemmable);
