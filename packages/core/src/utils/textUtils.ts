...
/**
 * Heuristically sanitizes unescaped newlines in string literals.
 * This handles cases where the LLM produces a literal newline
 * inside a double or single quoted string, causing a syntax error
 * when written to code files.
 *
 * @param content The string content to sanitize.
 * @param filePath The destination file path to check extension.
 * @returns The sanitized string.
 */
export function sanitizeBackslashes(content: string, filePath: string): string {
  if (!filePath) {
    return content;
  }
  ...
  // Regex matches "..." or '...' and handles escaped quotes inside
  const stringLiteralRegex = /("([^"\\]|\\.)*")|('([^'\\]|\\.)*')/g;

  return content.replace(stringLiteralRegex, (match) => {
    if (match.includes('\n') || match.includes('\r')) {
      return match
        .replace(/\r\n/g, '\\n')
        .replace(/\n/g, '\\n')
