...
describe('user_id', () => {
  ...
  describe('getGoogleAccountEmail', () => {
    it('should return a non-empty string', async () => {
      const result = await getGoogleAccountEmail();

      expect(result).toBeDefined();
      expect(typeof result).toBe('string');

      // Should be consistent on subsequent calls
      const secondCall = await getGoogleAccountEmail();
      expect(secondCall).toBe(result);
    });

    it('should return empty string when no Google Account email is cached', async () => {
      // In a clean test environment, there should be no cached Google Account email
      const googleAccountEmailResult = await getGoogleAccountEmail();
