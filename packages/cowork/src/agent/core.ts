...
export class Coworker {
  ...
  constructor(
    ...
    if (useMemory) {
      this.memoryStore = new MemoryStore(
        join(projectRoot, '.cowork', 'memory.json'),
      );
    ...
    if (feedbackPath || useMemory) {
      const path =
        feedbackPath ?? join(projectRoot, '.cowork', 'feedback.jsonl');
  ...
  private buildContext(): string {
    ...
    const mcpStr =
      mcpTools.length > 0
        ? `${mcpTools.length} tools (${mcpTools
            .map((t) => t.qualifiedName)
            .slice(0, 3)
            .join(', ')}${mcpTools.length > 3 ? '…' : ''})`
        : chalk.dim('none');
    ...
    if (this.memory.contextSummary) {
      const preview = this.memory.contextSummary
        .slice(0, 280)
        .replace(/\n/g, ' ');
  ...
  private async scanEnvironment(): Promise<void> {
    ...
    try {
      ...
      this.tracer?.record({
        phase: 'env_scan',
        content: 'Codebase indexing skipped.',
      });
    }
  }
  ...
  private async think(iteration: number): Promise<ToolCall | null> {
    ...
    if (this.memoryRetriever) {
      try {
        memoryContext = await this.memoryRetriever.retrieve(
          this.memory.goal,
          3,
        );
      } catch {
    ...
    if (iteration === 2 && SEARCH_KEYWORDS.test(this.memory.goal)) {
      this.record(
        'think',
        `Strategy → goal requires web lookup. Running search: "${this.memory.goal}"`,
      );
      return {
        tool: 'search',
    ...
    if (iteration === 2 && MONITOR_KEYWORDS.test(this.memory.goal)) {
      ...
      this.record(
        'think',
        `Strategy → goal involves monitoring. Starting: "${command}"`,
      );
      return {
        tool: 'log_monitor',
    ...
    this.record(
      'think',
      'Goal analysis complete. No further actions required.',
    );
    return null;
  }
      ...
      switch (toolCall.tool) {
        ...
        case 'auto_test': {
          ...
          result = {
            ...
            ...(healResult.success
              ? {}
              : { error: 'auto_test: tests did not pass.' }),
          };
          break;
        }
      }
  ...
  private async observeCodeowners(): Promise<void> {
    ...
    try {
      ...
      this.tracer?.record({
        phase: 'observe',
        content: `CODEOWNERS reviewer suggestions:\n${formatted}`,
      });
    } catch {
      // non-fatal
    }
  ...
  async runLoop(goal: string): Promise<void> {
    ...
    if (this.mcp && this.mcpServers.length > 0) {
      ...
      mcpSpinner.succeed(
        chalk.dim(
          `MCP: ${connected}/${this.mcpServers.length} server(s) connected.`,
        ),
      );
    }
