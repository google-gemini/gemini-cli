...
export const DirectoryPanel: React.FC = () => {
  ...
  const handleWorkspaceSelect = async (workspace: typeof workspaces[0]) => {
    try {
      ...
      console.log(`Added directories from "${workspace.name}" to working folders:`, workspace.directories);
    } catch (error) {
      console.error('Failed to add workspace directories:', error);
    }
  };
  ...
  const handleRemoveWorkspace = async (workspaceToRemove: WorkspaceConfig, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent workspace selection when clicking delete
    
    try {
      // Remove workspace from store
      removeWorkspace(workspaceToRemove.id);
      
      console.log('Removed workspace:', workspaceToRemove.name);
    } catch (error) {
    ...
    <div className="p-4 h-full flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between mb-3 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">Working Folders</span>
        </div>
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="space-y-1 mb-3 flex-1 overflow-y-auto min-h-0">
        {directoriesLoading && (
      ...
      {/* Add Directory Form */}
      {showAddForm && (
        <div className="space-y-2 p-2 bg-accent/10 rounded border mt-auto">
          {/* Quick Browse Button */}
          <Button
            variant="outline"
            size="sm"
      ...
      {workspaces.length > 0 && (
        <div className="mt-4 pt-3 border-t border-border">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-muted-foreground">Recent Folders</span>
          </div>
          <div className="space-y-1">
            {workspaces
              ...
              .map((workspace) => (
                <div
                  key={workspace.id}
                  className="flex items-center gap-2 p-1.5 rounded text-xs cursor-pointer hover:bg-accent/30 transition-colors group"
                  onClick={() => handleWorkspaceSelect(workspace)}
                  title="Click to add folders to current workspace"
                >
                  ...
                  <div className="flex items-center gap-1">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={(e) => handleRemoveWorkspace(workspace, e)}
                      className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-destructive/20 hover:text-destructive"
