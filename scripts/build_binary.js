...
function removeSignature(filePath) {
  ...
  try {
    if (platform === 'darwin') {
      execSync(`codesign --remove-signature "${filePath}"`, {
        stdio: 'ignore',
      });
    } else if (platform === 'win32') {
      execSync(`signtool remove /s "${filePath}"`, { stdio: 'ignore' });
    }
  } catch {
    // Best effort: Ignore failures (file might not be signed, or tool missing)
  }
}

/**
 * Signs a binary using the appropriate tool for the platform.
 * @param {string} filePath
 */
function signFile(filePath) {
  ...
  if (platform === 'darwin') {
    ...
    // Use entitlements if available
    let entitlementsFlag = '';
    if (existsSync(entitlementsPath)) {
      entitlementsFlag = `--entitlements "${entitlementsPath}"`;
    }

    try {
      execSync(
        `codesign --sign "${identity}" ${entitlementsFlag} --force --timestamp --options runtime "${filePath}"`,
        { stdio: 'inherit' },
      );
    } catch (e) {
      throw new Error(`Codesign failed: ${e.message}`);
    }
  } else if (platform === 'win32') {
    let signCmd = process.env.SIGN_CMD_WIN;
    let hasSigntool = true; // Assume we have signtool if user is providing custom cmd
    if (!signCmd) {
      try {
      ...
      if (process.env.WINDOWS_PFX_FILE && process.env.WINDOWS_PFX_PASSWORD) {
        signCmd = `signtool sign /f "${process.env.WINDOWS_PFX_FILE}" /p "${process.env.WINDOWS_PFX_PASSWORD}" /fd SHA256 /td SHA256 /tr http://timestamp.digicert.com`;
      } else {
        signCmd =
    ...
    if (signCmd && hasSigntool) {
      console.log(`Signing ${filePath}...`);
      try {
        execSync(`${signCmd} "${filePath}"`, { stdio: 'inherit' });
      } catch (e) {
        let msg = e.message;
      ...
      console.log(`Skipping signing for ${filePath} (Tools not found)`);
    }
  } else if (platform === 'linux') {
    const signCmd = process.env.SIGN_CMD_LINUX;
    if (signCmd) {
      console.log(`Signing ${filePath}...`);
      try {
        execSync(`${signCmd} "${filePath}"`, { stdio: 'inherit' });
      } catch (e) {
        throw new Error(`Linux signing failed: ${e.message}`);
      }
    }
  }
}
