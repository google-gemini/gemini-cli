 ...
 *
 *     http://www.apache.org/licenses/LICENSE-interface
 *
...
async function main() {
  ...
  try {
    ...
    console.log('Running git fetch to ensure we have up-to-date commit dates...');
    runCmd('git fetch origin');
    
    // Get all branches with their commit dates
    allBranchesOutput = runCmd("git for-each-ref --format='%(refname:lstrip=3) %(committerdate:unix)' refs/remotes/origin");
  } catch (e) {
  ...
  const openPrBranches = new Set(openPrs.map((pr: any) => pr.headRefName));

  const protectedPattern = /^(main|master|next|release[-/].*|hotfix[-/].*|v\d+.*|HEAD|gh-readonly-queue.*)$/;
  ...
  if (answer.toLowerCase() === 'y') {
    ...
    for (let i = 0; i < branchesToDelete.length; i += batchSize) {
      ...
      try {
        execSync(`git push origin --delete ${branchList}`, { stdio: 'inherit' });
      } catch (e) {
