...
function readApiKeys(filePath) {
  try {
    ...
    if (keys.length === 0) {
      console.error(
        `${NR}No keys found in the scroll. Ensure they are formatted as '- "YOUR_KEY"'${RST}`,
      );
    }
    return keys;
  } catch (error) {
    console.error(
      `${NR}Failed to read the scroll of keys at ${filePath}: ${error.message}${RST}`,
    );
    return [];
  }
}
...
function getNextKey(keys, currentKey) {
  ...
  if (!currentKey) {
    console.log(
      `${NY}No current key detected. Selecting the first key from the scroll.${RST}`,
    );
    return keys[0];
  }
  ...
  if (currentIndex === -1) {
    console.log(
      `${NY}Current key not found in the scroll. Selecting the first key.${RST}`,
    );
    return keys[0];
  }
...
function cycleApiKey() {
  ...
  if (nextKey) {
    console.log(
      `${NG}Next key selected. To apply it to your session, run the following command:${RST}`,
    );
