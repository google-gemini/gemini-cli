...
import { quote } from 'node:shell-quote';
...
async function main() {
  ...
  if (testPatternIndex !== -1) {
    ...
    if (manualModel) {
      ...
      processResults(firstPass, pattern, manualModel, files);
      return;
    }
  }
  ...
  processResults(firstPass, pattern, model, files);
}
