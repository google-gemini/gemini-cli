...
export function replaceWithBackup(filePath: string, oldPattern: string, newText: string, config: any): Promise<string> {
                    ...
                    if (err) {
                        ...
                        fs.readFile(resolvedPath, 'utf-8', (err, data) => {
                            if (err) {
                                ...
                                if (!regex.test(data)) {
                                    reject(new Error(`Pattern '${oldPattern}' not found in ${filePath}`));
                                } else {
                                    ...
                                    fs.writeFile(resolvedPath, newContent, 'utf-8', (err) => {
                                        if (err) {
                                            ...
                                            resolve(`Replaced pattern in ${filePath}, backup at ${backupPath}`);
                                        }
                                    });
                                }
                            }
...
export function replaceRollback(filePath: string, config: any): Promise<string> {
    return new Promise((resolve, reject) => {
        checkFilePermission(filePath, "write", config)
            .then(() => {
                const resolvedPath = path.resolve(filePath);
