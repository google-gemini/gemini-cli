...
export function editWithUndo(filePath: string, editFunc: (filePath: string, config: any, ...args: any[]) => Promise<string>, config: any, ...args: any[]): Promise<string> {
                ...
                fs.copyFile(resolvedPath, undoPath, (err) => {
                    if (err) {
                        ...
                        editFunc(filePath, config, ...args)
                            .then(() => {
                                resolve(`Edit applied to ${filePath}, undo available at ${undoPath}`);
                            })
                            .catch(reject);
                    }
                });
