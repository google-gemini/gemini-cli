...
export function replaceNaturalLanguage(
  filePath: string,
  query: string,
  config: any,
): Promise<string> {
  return new Promise((resolve, reject) => {
    checkFilePermission(filePath, 'write', config)
      .then(() => {
        ...
        const [oldPattern, newText] = editMap[query.toLowerCase()] || [
          query,
          query,
        ];
        fs.readFile(resolvedPath, 'utf-8', (err, data) => {
          if (err) {
            ...
            if (!regex.test(data)) {
              reject(
                new Error(`Pattern for '${query}' not found in ${filePath}`),
              );
            } else {
              ...
              fs.writeFile(resolvedPath, newContent, 'utf-8', (err) => {
                if (err) {
                  ...
                  resolve(
                    `Applied natural language replacement '${query}' in ${filePath}`,
                  );
                }
              });
            }
