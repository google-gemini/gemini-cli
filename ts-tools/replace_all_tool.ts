...
export async function replaceAll(dirPath: string, pattern: string, replacement: string, config: any, regex: boolean = false): Promise<string> {
    ...
    for await (const p of walk(resolvedPath)) {
        ...
        const newContent = regex ? content.replace(new RegExp(pattern, 'g'), replacement) : content.replace(pattern, replacement);
