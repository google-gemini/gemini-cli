...
export function splitFile(filePath: string, linesPerFile: number, prefix: string, config: any): Promise<string> {
                ...
                fs.readFile(resolvedPath, 'utf-8', (err, data) => {
                    if (err) {
                        ...
                        for (let i = 0; i < lines.length; i += linesPerFile) {
                            const outputPath = path.join(path.dirname(resolvedPath), `${prefix}${fileCount}.txt`);
