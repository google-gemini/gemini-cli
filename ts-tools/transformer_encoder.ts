...
class PositionalEncoding extends nn.Module {
    ...
    constructor(d_model: number, max_len: number = 5000) {
        ...
        const position = torch.arange(0, max_len, { dtype: torch.float }).unsqueeze(1);
        const div_term = torch.exp(torch.arange(0, d_model, 2).float().mul(-math.log(10000.0) / d_model));
...
class MultiHeadAttention extends nn.Module {
    ...
    forward(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, mask?: torch.Tensor): torch.Tensor {
        ...
        q = q.view(query.shape[0], -1, this.n_heads, this.head_dim).permute(0, 2, 1, 3);
        k = k.view(key.shape[0], -1, this.n_heads, this.head_dim).permute(0, 2, 1, 3);
        v = v.view(value.shape[0], -1, this.n_heads, this.head_dim).permute(0, 2, 1, 3);
        const scores = torch.matmul(q, k.permute(0, 1, 3, 2)).div(math.sqrt(this.head_dim));
        if (mask) {
            scores.masked_fill_(mask.eq(0), -1e9);
        }
        const attention = torch.softmax(scores, -1);
        let output = torch.matmul(attention, v);
        output = output.permute(0, 2, 1, 3).contiguous().view(query.shape[0], -1, this.d_model);
...
export class TransformerEncoder extends nn.Module {
    ...
    constructor(vocab_size: number, d_model: number, n_layers: number, n_heads: number, d_ff: number) {
        super();
        this.embedding = new nn.Embedding(vocab_size, d_model);
        this.pos_encoding = new PositionalEncoding(d_model);
        this.layers = new nn.ModuleList(Array.from({ length: n_layers }, () => new EncoderLayer(d_model, n_heads, d_ff)));
    }
